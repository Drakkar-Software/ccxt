...
impl serde::Serialize for Value {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        match self {
            Value::Undefined => s.serialize_none(),
...
pub trait Exchange: ValueTrait {
    ...
    fn url_encoder_for_proxy_url(&self, mut target_url: Value) -> Value { Value::Undefined }
    ...
    /// * `params` {object} - 
    fn calculate_fee(&self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut taker_or_maker: Value, mut params: Value) -> Value { Value::Undefined }
    ...
    fn create_ccxt_trade_id(&self, mut timestamp: Value, mut side: Value, mut amount: Value, mut price: Value, mut taker_or_maker: Value) -> Value { Value::Undefined }
...
// ---------------------------------------------------------------------------
// Response unwrapping helpers — used by transpiler-generated fetch_* templates
// that need to extract the inner array (or order-book object) from common
// exchange-response wrapper shapes before parsing.
// ---------------------------------------------------------------------------

/// Try to extract the inner JSON array from a wrapped exchange response.
/// Handles common shapes: bare array, `{data: [...]}`, `{result: {list: [...]}}`,
/// `{result: {data: [...]}}`, `{data: {data: [...]}}`, etc.
/// Returns the bare response unchanged if no wrapper is detected.
pub fn unwrap_response_array(response: &serde_json::Value) -> serde_json::Value {
    if response.is_array() { return response.clone(); }
    let candidate_keys = ["data", "result", "klines", "candles", "ohlcv", "trades", "list", "rows", "items"];
    for key in &candidate_keys {
        if let Some(v) = response.get(key) {
            if v.is_array() { return v.clone(); }
            for nested in &["list", "data", "rows", "items", "ohlcv"] {
                if let Some(arr) = v.get(nested) {
                    if arr.is_array() { return arr.clone(); }
                }
            }
        }
    }
...
/// Try to extract an order-book object (containing both `bids` and `asks` keys).
/// Returns the bare response unchanged if no wrapper is detected.
pub fn unwrap_response_order_book(response: &serde_json::Value) -> serde_json::Value {
    if response.get("bids").is_some() && response.get("asks").is_some() {
