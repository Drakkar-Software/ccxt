...
pub trait Ndax : Exchange {
    ...
    fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < payload.len() {
            ...
            updates.set(symbol.clone(), Value::from(true));
            i += 1;
        };
    ...
    fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < payload.len() {
            ...
            while j < keys.len() {
                ...
                let mut stored: Value = self.safe_value(self.get("ohlcvs".into()).get(symbol.clone().clone()), timeframe.clone(), Value::new_array());
                let mut length: usize = stored.len();
                if length != 0 && parsed.get(Value::from(0)) == stored.get(Value::from(length) - Value::from(1).clone()).get(Value::from(0)) {
                    ...
                    updates.get(market_id.clone().clone()).set(timeframe.clone(), Value::from(true));
                } else {
                    if length != 0 && parsed.get(Value::from(0)) < stored.get(Value::from(length) - Value::from(1).clone()).get(Value::from(0)) {
                        continue;
                    } else {
        ...
        while i < market_ids.len() {
            ...
            let mut timeframes: Value = updates.get(market_id.clone().clone()).keys();
            let mut j: usize = 0;
            while j < timeframes.len() {
