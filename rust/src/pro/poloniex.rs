...
signTimestamp=") + timestamp.clone();
    ...
    async fn subscribe(&mut self, mut name: Value, mut message_hash: Value, mut is_private: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        let mut url: Value = self.get("urls".into()).get(Value::from("api")).get(Value::from("ws")).get(public_or_private.clone().clone());
    ...
    async fn watch_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut ohlcv: Value = <Self as Poloniex>::subscribe(self, channel.clone(), channel.clone(), Value::from(false), Value::Json(serde_json::Value::Array(vec![(symbol.clone()).into()])), params.clone()).await;
    ...
    async fn watch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        return self.safe_value(tickers.clone(), symbol.clone(), Value::Undefined).await;
    }
    ...
    async fn watch_tickers(&mut self, mut symbols: Value, mut params: Value) -> Value {
        ...
        let mut new_tickers: Value = <Self as Poloniex>::subscribe(self, name.clone(), name.clone(), Value::from(false), symbols.clone(), params.clone()).await;
        if self.get("newUpdates".into()).is_truthy() {
    ...
    async fn watch_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut orderbook: Value = <Self as Poloniex>::subscribe(self, name.clone(), name.clone(), Value::from(false), Value::Json(serde_json::Value::Array(vec![(symbol.clone()).into()])), params.clone()).await;
    ...
    async fn watch_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut orders: Value = <Self as Poloniex>::subscribe(self, name.clone(), name.clone(), Value::from(true), symbols.clone(), params.clone()).await;
        if self.get("newUpdates".into()).is_truthy() {
    ...
    async fn watch_my_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut symbols: Value = if symbol.clone().is_nullish() { Value::Undefined } else { Value::Json(serde_json::Value::Array(vec![(symbol.clone()).into()])) };
        let mut trades: Value = <Self as Poloniex>::subscribe(self, name.clone(), message_hash.clone(), Value::from(true), symbols.clone(), params.clone()).await;
        if self.get("newUpdates".into()).is_truthy() {
    ...
    async fn watch_balance(&mut self, mut params: Value) -> Value {
        ...
        <Self as Poloniex>::authenticate(self, Value::Undefined).await;
        return <Self as Poloniex>::subscribe(self, name.clone(), name.clone(), Value::from(true), Value::Undefined, params.clone()).await;
    }
