...
pub trait Apex : Exchange {
    ...
    fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while j < length {
            ...
            let mut parsed: Value = <Self as Apex>::parse_ws_trade(self, trades.get(index.clone().clone()), market.clone());
    ...
    async fn load_positions_snapshot(&mut self, mut client: Value, mut message_hash: Value) -> Value {
        ...
        let mut fetch_functions: Value = Value::Json(serde_json::Value::Array(vec![(self.fetch_positions(Value::Undefined, Value::Undefined).await).into()]));
