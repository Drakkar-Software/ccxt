...
pub trait Bitmart : Exchange {
    ...
    async fn subscribe(&mut self, mut channel: Value, mut symbol: Value, mut r#type: Value, mut params: Value) -> Value {
        ...
        let mut url: Value = self.implode_hostname(self.get("urls".into()).get(Value::from("api")).get(Value::from("ws")).get(r#type.clone().clone()).get(Value::from("public")));
        let mut request: Value = Value::new_object();
        let mut message_hash: Value = Value::Undefined;
        let mut unsubscribe: Value = self.safe_bool(params.clone(), Value::from("unsubscribe"), Value::from(false));
    ...
    async fn subscribe_multiple(&mut self, mut channel: Value, mut r#type: Value, mut symbols: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        symbols = self.market_symbols(symbols.clone(), r#type.clone(), Value::from(false), Value::from(true), Value::Undefined);
        let mut url: Value = self.implode_hostname(self.get("urls".into()).get(Value::from("api")).get(Value::from("ws")).get(r#type.clone().clone()).get(Value::from("public")));
    ...
    async fn watch_balance(&mut self, mut params: Value) -> Value {
        ...
        let mut url: Value = self.implode_hostname(self.get("urls".into()).get(Value::from("api")).get(Value::from("ws")).get(r#type.clone().clone()).get(Value::from("private")));
        ...
        (fetch_balance_snapshot, params) = shift_2(self.handle_option_and_params(params.clone(), Value::from("watchBalance"), Value::from("fetchBalanceSnapshot"), Value::from(true)));
        (await_balance_snapshot, params) = shift_2(self.handle_option_and_params(params.clone(), Value::from("watchBalance"), Value::from("awaitBalanceSnapshot"), Value::from(false)));
    ...
    fn set_balance_cache(&mut self, mut client: Value, mut r#type: Value, mut subscribe_hash: Value) -> Value {
        if client.get("subscriptions".into()).contains_key(subscribe_hash.clone()) {
            return Value::Undefined;
        };
        let mut options: Value = self.safe_dict(self.get("options".into()), Value::from("watchBalance"), Value::Undefined);
        let mut snapshot: Value = self.safe_bool(options.clone(), Value::from("fetchBalanceSnapshot"), Value::from(true));
    ...
    async fn load_balance_snapshot(&mut self, mut client: Value, mut message_hash: Value, mut r#type: Value) -> Value {
        ...
        if client.get("futures".into()).contains_key(message_hash.clone()) {
            ...
            client.resolve(self.get("balance".into()).get(r#type.clone().clone()), Value::from("balance:") + r#type.clone());
        };
        Value::Undefined
    }
    ...
    fn handle_balance(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        self.get("balance".into()).get(r#type.clone().clone()).set("info".into(), message.clone());
        if is_spot.is_truthy() {
            ...
            while i < data.len() {
                ...
                self.get("balance".into()).get(r#type.clone().clone()).set("timestamp".into(), timestamp.clone());
                self.get("balance".into()).get(r#type.clone().clone()).set("datetime".into(), self.iso8601(timestamp.clone()));
                ...
                while ii < balance_details.len() {
                    ...
                    self.get("balance".into()).get(r#type.clone().clone()).set(code.clone(), account.clone());
                    ii += 1;
                };
                i += 1;
            };
        ...
        self.get("balance".into()).set(r#type.clone(), self.safe_balance(self.get("balance".into()).get(r#type.clone().clone())));
        let mut message_hash: Value = Value::from("balance:") + r#type.clone();
        client.resolve(self.get("balance".into()).get(r#type.clone().clone()), message_hash.clone());
        Value::Undefined
    }
    ...
    async fn watch_bids_asks(&mut self, mut symbols: Value, mut params: Value) -> Value {
        ...
        let mut url: Value = self.implode_hostname(self.get("urls".into()).get(Value::from("api")).get(Value::from("ws")).get(market_type.clone().clone()).get(Value::from("public")));
    ...
    async fn watch_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.filter_by_symbol_since_limit(self.get("orders".into()), symbol.clone(), since.clone(), limit.clone(), Value::from(true)).await;
    }
    ...
    async fn watch_positions(&mut self, mut symbols: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        <Self as Bitmart>::authenticate(self, r#type.clone(), params.clone()).await;
        symbols = self.market_symbols(symbols.clone(), Value::from("swap"), Value::from(true), Value::from(true), Value::from(false));
        ...
        return self.filter_by_symbols_since_limit(self.get("positions".into()), symbols.clone(), since.clone(), limit.clone(), Value::Undefined).await;
    }
    ...
    fn handle_trade(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if is_swap.is_truthy() {
            ...
            while i < length {
                ...
                symbol = <Self as Bitmart>::handle_trade_loop(self, data.get(index.clone().clone()));
                i += 1;
            };
        } else {
        ...
        client.resolve(self.get("trades".into()).get(symbol.clone().clone()), Value::from("trade:") + symbol.clone());
        Value::Undefined
    }
    ...
    fn handle_ticker(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < raw_tickers.len() {
            let mut ticker: Value = if is_spot.is_truthy() { self.parse_ticker(raw_tickers.get(i.into()), Value::Undefined) } else { <Self as Bitmart>::parse_ws_swap_ticker(self, raw_tickers.get(i.into()), Value::Undefined) };
            let mut symbol: Value = ticker.get(Value::from("symbol"));
    ...
    fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if is_spot.is_truthy() {
            ...
            while i < data.len() {
                ...
                parsed.set(0.into(), self.parse_to_int(parsed.get(Value::from(0)) / duration_in_ms.clone()) * duration_in_ms.clone());
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if is_spot.is_truthy() {
            ...
            if way.clone() == Value::from(1) {
                orderbook.set(side.clone(), Bids::new(Value::new_array(), Value::from(limit)));
            } else {
                orderbook.set(side.clone(), Asks::new(Value::new_array(), Value::from(limit)));
            };
            let mut i: usize = 0;
            while i < depths.len() {
            ...
            let mut bids_length: usize = orderbook.get(Value::from("bids")).len();
            let mut asks_length: usize = orderbook.get(Value::from("asks")).len();
            if bids_length == Value::from(0) || asks_length == Value::from(0) {
                return Value::Undefined;
            };
    ...
    fn handle_error_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut error: Value = self.safe_string(message.clone(), Value::from("error"), Value::Undefined);
            ...
            if action.clone() == Value::from("access") {
                panic!("{}", r###"AuthenticationError::new(feedback.clone())"###);
            };
...
;
    ...
    fn handle_un_subscription(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut sub_hash_is_prefix: Value = self.safe_bool(subscription.clone(), Value::from("subHashIsPrefix"), Value::from(false));
    ...
    fn get_un_sub_params(&mut self, mut message_topic: Value) -> Value {
        ...
        let mut sub_hash_is_prefix: Value = Value::from(false);
    ...
    fn parse_topic(&mut self, mut topic: Value) -> Value {
        if topic.starts_with(Value::from("depth")) {
