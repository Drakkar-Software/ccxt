...
pub trait Alpaca : Exchange {
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut is_snapshot: Value = self.safe_bool(message.clone(), Value::from("r"), Value::from(false));
    ...
    fn handle_crypto_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < message.len() {
            ...
            if T.clone() == Value::from("subscription") {
                <Self as Alpaca>::handle_subscription(self, client.clone(), data.clone());
                return Value::Undefined;
            };
            if T.clone() == Value::from("success") && msg.clone() == Value::from("connected") {
                <Self as Alpaca>::handle_connected(self, client.clone(), data.clone());
                return Value::Undefined;
            };
            if T.clone() == Value::from("success") && msg.clone() == Value::from("authenticated") {
            ...
            let mut method: Value = self.safe_value(methods.clone(), T.clone(), Value::Undefined);
    ...
    fn handle_authenticate(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if T.clone() == Value::from("success") || status.clone() == Value::from("authorized") {
            let mut promise: Value = client.get("futures".into()).get(Value::from("authenticated"));
