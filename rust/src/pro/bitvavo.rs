...
pub trait Bitvavo : Exchange {
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if orderbook.get(Value::from("nonce")).is_nullish() {
            ...
            if watching_order_book_snapshot.clone().is_nullish() {
                subscription.set("watchingOrderBookSnapshot".into(), Value::from(true));
                ...
                self.delay(delay.clone(), self.get("watchOrderBookSnapshot".into()), client.clone());
            };
    ...
    async fn fetch_my_trades_ws(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.filter_by_symbol_since_limit(my_trades.clone(), symbol.clone(), since.clone(), limit.clone(), Value::Undefined).await;
    }
    ...
    async fn fetch_withdrawals_ws(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.filter_by_currency_since_limit(withdraws.clone(), code.clone(), since.clone(), limit.clone(), Value::Undefined).await;
    }
    ...
    async fn fetch_deposits_ws(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.filter_by_currency_since_limit(deposits.clone(), code.clone(), since.clone(), limit.clone(), Value::Undefined).await;
    }
    ...
    fn handle_authentication_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        //
        let mut message_hash: Value = Value::from("authenticated");
        let mut authenticated: Value = self.safe_bool(message.clone(), Value::from("authenticated"), Value::from(false));
    ...
    fn handle_error_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut rejected: Value = Value::from(false);
                self.handle_errors(code.clone(), error.clone(), client.get("url".into()), Value::from(""), Value::new_object(), error.clone(), message.clone(), Value::new_object(), Value::new_object());
