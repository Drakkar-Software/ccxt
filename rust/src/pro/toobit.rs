...
pub trait Toobit : Exchange {
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        //     }
        //
        let mut is_snapshot: Value = self.safe_bool(message.clone(), Value::from("f"), Value::from(false));
        ...
        while i < data.len() {
            ...
            let mut order_book: Value = self.get("orderbooks".into()).get(symbol.clone().clone());
    ...
    async fn load_balance_snapshot(&mut self, mut client: Value, mut message_hash: Value, mut market_type: Value) -> Value {
        ...
        if client.get("futures".into()).contains_key(message_hash.clone()) {
            ...
            client.resolve(self.get("balance".into()).get(r#type.clone().clone()), r#type.clone() + Value::from(":fetchBalanceSnapshot"));
            client.resolve(self.get("balance".into()).get(r#type.clone().clone()), r#type.clone() + Value::from(":balance"));
        };
        Value::Undefined
    }
    ...
    fn parse_my_trade(&mut self, mut trade: Value, mut market: Value) -> Value {
...
;
    ...
    fn get_user_stream_url(&self) -> Value {
    ...
    fn handle_error_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if code.clone().is_nonnullish() {
            ...
            let mut exception: Value = ExchangeError::new(msg.clone());
            // c# fix
            client.reject(exception.clone(), Value::Undefined);
