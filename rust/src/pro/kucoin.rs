...
;
    ...
    async fn watch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut url: Value = <Self as Kucoin>::negotiate(self, Value::from(false), Value::Undefined).await;
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if subject.clone() == Value::from("level2") {
            if !self.get("orderbooks".into()).contains_key(symbol.clone()) {
                ...
                orderbook.reset(Value::Undefined);
            };
            self.get("orderbooks".into()).get(symbol.clone().clone()).set("symbol".into(), symbol.clone());
        } else {
            if !self.get("orderbooks".into()).contains_key(symbol.clone()) {
                self.get("orderbooks".into()).set(symbol.clone(), self.order_book(Value::Undefined, Value::Undefined));
            };
        ...
        <Self as Kucoin>::handle_delta(self, self.get("orderbooks".into()).get(symbol.clone().clone()), data.clone());
    ...
    async fn watch_orders(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut url: Value = <Self as Kucoin>::negotiate(self, Value::from(true), Value::Undefined).await;
    ...
    fn parse_ws_order(&self, mut order: Value, mut market: Value) -> Value {
        ...
        let mut trigger_fail: Value = (trigger_success.clone() != Value::from(true) && trigger_success.clone().is_nonnullish()).into();
    ...
    fn parse_ws_trade(&self, mut trade: Value, mut market: Value) -> Value {
        ...
        let mut timestamp: Value = self.safe_integer_product_2(trade.clone(), Value::from("ts"), Value::from("time"), Value::from(0.000001), Value::Undefined);
    ...
    fn handle_balance(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        self.get("balance".into()).get(uniform_type.clone().clone()).set("info".into(), data.clone());
        let mut timestamp: Value = self.safe_integer(data.clone(), Value::from("time"), Value::Undefined);
        self.get("balance".into()).get(uniform_type.clone().clone()).set("timestamp".into(), timestamp.clone());
        self.get("balance".into()).get(uniform_type.clone().clone()).set("datetime".into(), self.iso8601(timestamp.clone()));
        ...
        self.get("balance".into()).get(uniform_type.clone().clone()).set(code.clone(), account.clone());
        self.get("balance".into()).set(uniform_type.clone(), self.safe_balance(self.get("balance".into()).get(uniform_type.clone().clone())));
        if uniform_type.clone() == selected_type.clone() {
            client.resolve(self.get("balance".into()).get(uniform_type.clone().clone()), message_hash.clone());
        };
        Value::Undefined
    }
