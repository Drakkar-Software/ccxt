...
pub trait Bittrade : Exchange {
    ...
    async fn watch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut url: Value = self.implode_params(self.get("urls".into()).get(Value::from("api")).get(Value::from("ws")).get(api.clone().clone()).get(Value::from("public")), hostname.clone());
