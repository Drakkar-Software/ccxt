...
pub trait Whitebit : Exchange {
    ...
    fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < params.len() {
            ...
            if !self.get("ohlcvs".into()).get(symbol.clone().clone()).contains_key(Value::from("unknown")) {
            ...
            let mut ohlcv: Value = self.get("ohlcvs".into()).get(symbol.clone().clone()).get(Value::from("unknown"));
    ...
    async fn watch_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut req_params: Value = Value::Json(serde_json::Value::Array(vec![(market.get(Value::from("id"))).into(), (limit.clone()).into(), (price_interval.clone()).into(), (Value::from(true)).into()]));
    ...
    async fn watch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        return <Self as Whitebit>::watch_multiple_subscription(self, message_hash.clone(), method.clone(), symbol.clone(), Value::from(false), params.clone()).await;
    }
    ...
    fn handle_ticker(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        // watchTickers
        let mut message_hashes: Value = client.get("futures".into()).keys();
        let mut i: usize = 0;
    ...
    async fn watch_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        // every time we want to subscribe to another market we have to 're-subscribe' sending it all again
        let mut trades: Value = <Self as Whitebit>::watch_multiple_subscription(self, message_hash.clone(), method.clone(), symbol.clone(), Value::from(false), params.clone()).await;
        if self.get("newUpdates".into()).is_truthy() {
    ...
    async fn watch_my_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut trades: Value = <Self as Whitebit>::watch_multiple_subscription(self, message_hash.clone(), method.clone(), symbol.clone(), Value::from(true), params.clone()).await;
        if self.get("newUpdates".into()).is_truthy() {
            limit = trades.get_limit(symbol.clone(), limit.clone());
        };
    ...
    async fn watch_multiple_subscription(&mut self, mut message_hash: Value, mut method: Value, mut symbol: Value, mut is_nested: Value, mut params: Value) -> Value {
        is_nested = is_nested.or_default(Value::from(false));
        params = params.or_default(Value::new_object());
        self.load_markets(Value::Undefined, Value::Undefined).await;
        let mut url: Value = self.get("urls".into()).get(Value::from("api")).get(Value::from("ws"));
        let mut id: Value = self.nonce();
        ...
        if client.clone().is_nullish() {
            ...
            let mut subscription: Value = self.safe_value(client.get("subscriptions".into()), method.clone(), Value::new_object());
            let mut has_symbol_subscription: Value = Value::from(true);
            let mut market: Value = self.market(symbol.clone());
            let mut market_id: Value = market.get(Value::from("id"));
            let mut is_subscribed: Value = self.safe_bool(subscription.clone(), market_id.clone(), Value::from(false));
            if !is_subscribed.is_truthy() {
                subscription.set(market_id.clone(), Value::from(true));
                has_symbol_subscription = Value::from(false);
            };
            if has_symbol_subscription.is_truthy() {
                ...
                if client.get("subscriptions".into()).contains_key(method.clone()) {
                    client.get("subscriptions".into()).get(method.clone().clone());
                };
...
;
    ...
    fn handle_authenticate(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        //
        let mut future: Value = client.get("futures".into()).get(Value::from("authenticated"));
        future.resolve(Value::from(1), Value::Undefined);
        return message.clone();
    }
...
;
    ...
    fn handle_subscription_status(&mut self, mut client: Value, mut message: Value, mut id: Value) -> Value {
        ...
        let mut subs: Value = client.get("subscriptions".into());
        let mut values: Value = subs.clone().values();
        let mut i: usize = 0;
        while i < values.len() {
            let mut subscription: Value = values.get(i.into());
            if subscription.clone() != Value::from(true) {
