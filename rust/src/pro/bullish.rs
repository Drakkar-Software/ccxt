...
pub trait Bullish : Exchange {
    ...
    async fn watch_private(&mut self, mut message_hash: Value, mut subscribe_hash: Value, mut request: Value, mut params: Value) -> Value {
        ...
        let mut token: Value = self.handle_token(Value::Undefined, Value::Undefined).await;
    ...
    fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if !self.get("trades".into()).contains_key(symbol.clone()) {
            ...
            let mut trades_array_cache: Value = ArrayCache::new();
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if sequence_number_range.len() > Value::from(0) {
            let mut last_index: Value = Value::from(sequence_number_range.len()) - Value::from(1);
    ...
    fn handle_orders(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if raw_orders.len() > Value::from(0) {
    ...
    fn handle_my_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if raw_trades.len() > Value::from(0) {
    ...
    fn handle_balance(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if message_type.clone() == Value::from("snapshot") {
            ...
            self.get("balance".into()).get(trading_account_id.clone().clone()).set(code.clone(), account.clone());
            self.get("balance".into()).get(trading_account_id.clone().clone()).set("info".into(), message.clone());
            self.get("balance".into()).set(trading_account_id.clone(), self.safe_balance(self.get("balance".into()).get(trading_account_id.clone().clone())));
        };
        let mut message_hash: Value = Value::from("balance");
        let mut trading_account_id_hash: Value = Value::from("::") + trading_account_id.clone();
        client.resolve(self.get("balance".into()).get(trading_account_id.clone().clone()), message_hash.clone());
        client.resolve(self.get("balance".into()).get(trading_account_id.clone().clone()), message_hash.clone() + trading_account_id_hash.clone());
        Value::Undefined
    }
    ...
    async fn watch_positions(&mut self, mut symbols: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.filter_by_symbols_since_limit(positions.clone(), symbols.clone(), since.clone(), limit.clone(), Value::from(true)).await;
    }
    ...
    fn handle_positions(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < message_hashes.len() {
            ...
            let mut symbol_positions: Value = self.filter_by_array(new_positions.clone(), Value::from("symbol"), symbols.clone(), Value::from(false));
