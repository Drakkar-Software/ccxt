...
pub trait Gate : Exchange {
    ...
    async fn create_order_ws(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        params.set("textIsRequired".into(), Value::from(true));
    ...
    async fn create_orders_ws(&mut self, mut orders: Value, mut params: Value) -> Value {
        ...
        if market.get(Value::from("swap")) != Value::from(true) {
        ...
        return self.parse_orders(raw_orders.clone(), market.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
    }
    ...
    async fn cancel_all_orders_ws(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let (mut request, mut request_params) = shift_2(if r#type.clone() == Value::from("spot") { self.multi_order_spot_prepare_request(market.clone(), trigger.clone()) } else { self.prepare_request(market.clone(), r#type.clone(), query.clone()) });
        <Self as Gate>::authenticate(self, url.clone(), message_type.clone()).await;
    ...
    async fn cancel_order_ws(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        self.load_markets(Value::Undefined, Value::Undefined).await;
        let mut market: Value = if symbol.clone().is_nullish() { Value::Undefined } else { self.market(symbol.clone()) };
        let mut trigger: Value = self.safe_value_n(params.clone(), Value::Json(serde_json::Value::Array(vec![(Value::from("is_stop_order")).into(), (Value::from("stop")).into(), (Value::from("trigger")).into()])), Value::from(false));
        ...
        let (mut request, mut request_params) = shift_2(if r#type.clone() == Value::from("spot") || r#type.clone() == Value::from("margin") { self.spot_order_prepare_request(market.clone(), trigger.clone(), query.clone(), Value::Undefined, Value::Undefined, Value::Undefined) } else { self.prepare_request(market.clone(), r#type.clone(), query.clone()) });
        let mut message_type: Value = <Self as Gate>::get_type_by_market(self, market.clone());
        ...
        return self.parse_order(res.clone(), market.clone()).await;
    }
    ...
    async fn edit_order_ws(&mut self, mut id: Value, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        return self.parse_order(raw_order.clone(), market.clone()).await;
    }
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut raw_market_type: Value = self.safe_string(channel_parts.clone(), Value::from(0), Value::Undefined);
        ...
        let mut delta: Value = self.safe_value(message.clone(), Value::from("result"), Value::Undefined);
        ...
        let mut market_id: Value = self.safe_string(delta.clone(), Value::from("s"), Value::Undefined);
        ...
        if nonce.clone().is_nullish() {
            ...
            if stored_order_book.clone().is_nonnullish() {
                cache_length = stored_order_book.get("cache".into()).len();
            };
            ...
            if cache_length == wait_amount.clone() {
                // max limit is 100
            ...
            stored_order_book.get("cache".into()).push(delta.clone());
    ...
    fn get_cache_index(&mut self, mut order_book: Value, mut cache: Value) -> Value {
        let mut nonce: Value = self.safe_integer(order_book.clone(), Value::from("nonce"), Value::Undefined);
    ...
    fn handle_bid_asks(&mut self, mut book_side: Value, mut bid_asks: Value) -> Value {
        ...
        while i < bid_asks.len() {
            ...
            if Array::is_array(bid_ask.clone()).is_truthy() {
                ...
                let mut price: Value = self.safe_float(bid_ask.clone(), Value::from("p"), Value::Undefined);
                let mut amount: Value = self.safe_float(bid_ask.clone(), Value::from("s"), Value::Undefined);
    ...
    fn handle_delta(&mut self, mut orderbook: Value, mut delta: Value) -> Value {
        let mut timestamp: Value = self.safe_integer(delta.clone(), Value::from("t"), Value::Undefined);
    ...
    async fn watch_ticker(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        return self.safe_value(result.clone(), symbol.clone(), Value::Undefined).await;
    }
    ...
    async fn subscribe_watch_tickers_and_bids_asks(&mut self, mut symbols: Value, mut caller_method_name: Value, mut params: Value) -> Value {
        ...
        return self.filter_by_array(result.clone(), Value::from("symbol"), symbols.clone(), Value::from(true)).await;
    }
    ...
    fn handle_ticker_and_bid_ask(&mut self, mut object_name: Value, mut client: Value, mut message: Value) -> Value {
        ...
        let mut raw_market_type: Value = self.safe_string(parts.clone(), Value::from(0), Value::Undefined);
        ...
        while i < results.len() {
            ...
            let mut market_id: Value = self.safe_string(raw_ticker.clone(), Value::from("s"), Value::Undefined);
    ...
    fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < parsed_trades.len() {
            ...
            let mut cached_trades: Value = self.safe_value(self.get("trades".into()), symbol.clone(), Value::Undefined);
    ...
    fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < result.len() {
            ...
            let mut timeframe_id: Value = self.safe_string(parts.clone(), Value::from(0), Value::Undefined);
        ...
        while i < keys.len() {
            ...
            let mut timeframe: Value = market_ids.get(symbol.clone().clone());
            ...
            let mut stored: Value = self.safe_value(self.get("ohlcvs".into()).get(symbol.clone().clone()), interval.clone(), Value::Undefined);
    ...
    fn handle_my_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if cached_trades.clone().is_nullish() {
            let mut limit: Value = self.safe_integer(self.get("options".into()), Value::from("tradesLimit"), Value::from(1000));
            cached_trades = ArrayCacheBySymbolById::new();
        ...
        while i < parsed.len() {
            ...
            cached_trades.append(trade.clone());
            let mut symbol: Value = trade.get(Value::from("symbol"));
            market_ids.set(symbol.clone(), Value::from(true));
            i += 1;
        };
        let mut keys: Value = market_ids.clone().keys();
        let mut i: usize = 0;
    ...
    fn handle_balance(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        //
        let mut result: Value = self.safe_value(message.clone(), Value::from("result"), Value::new_array());
        let mut timestamp: Value = self.safe_integer(message.clone(), Value::from("time_ms"), Value::Undefined);
        ...
        while i < result.len() {
            ...
            account.set("total".into(), self.safe_string_2(raw_balance.clone(), Value::from("total"), Value::from("balance"), Value::Undefined));
            self.get("balance".into()).set(code.clone(), account.clone());
            i += 1;
        };
        let mut channel: Value = self.safe_string(message.clone(), Value::from("channel"), Value::Undefined);
        let mut parts: Value = channel.split(Value::from("."));
        let mut raw_type: Value = self.safe_string(parts.clone(), Value::from(0), Value::Undefined);
    ...
    async fn watch_positions(&mut self, mut symbols: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut positions: Value = <Self as Gate>::subscribe_private(self, url.clone(), message_hash.clone(), payload.clone(), channel.clone(), query.clone(), Value::from(true)).await;
        if self.get("newUpdates".into()).is_truthy() {
            return positions.clone();
        };
        return self.filter_by_symbols_since_limit(self.get("positions".into()).get(r#type.clone().clone()), symbols.clone(), since.clone(), limit.clone(), Value::from(true)).await;
    }
    ...
    fn handle_positions(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        //    }
        //
        let mut r#type: Value = <Self as Gate>::get_market_type_by_url(self, client.get("url".into()));
    ...
    fn handle_order(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < parsed_orders.len() {
            ...
            // inject order status
            let mut info: Value = self.safe_value(parsed.clone(), Value::from("info"), Value::Undefined);
            let mut event: Value = self.safe_string(info.clone(), Value::from("event"), Value::Undefined);
            if event.clone() == Value::from("put") || event.clone() == Value::from("update") {
                ...
                let mut status: Value = self.safe_string(parsed.clone(), Value::from("status"), Value::Undefined);
                if status.clone().is_nullish() {
                    let mut left: Value = self.safe_integer(info.clone(), Value::from("left"), Value::Undefined);
            ...
            let mut market: Value = self.market(symbol.clone());
            market_ids.set(market.get(Value::from("id")), Value::from(true));
            i += 1;
        };
        let mut keys: Value = market_ids.clone().keys();
        let mut i: usize = 0;
    ...
    async fn watch_my_liquidations(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return <Self as Gate>::watch_my_liquidations_for_symbols(self, Value::Json(serde_json::Value::Array(vec![(symbol.clone()).into()])), since.clone(), limit.clone(), params.clone()).await;
    }
    ...
    async fn watch_my_liquidations_for_symbols(&mut self, mut symbols: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut new_liquidations: Value = <Self as Gate>::subscribe_private(self, url.clone(), message_hash.clone(), payload.clone(), channel.clone(), query.clone(), Value::from(true)).await;
        if self.get("newUpdates".into()).is_truthy() {
    ...
    fn parse_ws_liquidation(&self, mut liquidation: Value, mut market: Value) -> Value {
        ...
        let mut timestamp: Value = self.safe_integer(liquidation.clone(), Value::from("time_ms"), Value::Undefined);
        let mut original_size: Value = self.safe_string(liquidation.clone(), Value::from("size"), Value::Undefined);
        let mut left: Value = self.safe_string(liquidation.clone(), Value::from("left"), Value::Undefined);
    ...
    fn handle_error_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut code: Value = self.safe_string_2(error.clone(), Value::from("code"), Value::from("label"), Value::Undefined);
        let mut id: Value = self.safe_string_n(message.clone(), Value::Json(serde_json::Value::Array(vec![(Value::from("id")).into(), (Value::from("requestId")).into(), (Value::from("request_id")).into()])), Value::Undefined);
        if error.clone().is_nonnullish() {
            let mut message_hash: Value = self.safe_string(client.get("subscriptions".into()), id.clone(), Value::Undefined);
            ...
            let mut error_message: Value = self.safe_string(error.clone(), Value::from("message"), self.safe_string(errs.clone(), Value::from("message"), Value::Undefined));
...
;
            ...
            if id.clone().is_nonnullish() && client.get("subscriptions".into()).contains_key(id.clone()) {
    ...
    fn handle_subscription_status(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if methods.contains_key(channel.clone()) {
            let mut subscription_hash: Value = self.safe_string(client.get("subscriptions".into()), id.clone(), Value::Undefined);
            let mut subscription: Value = self.safe_value(client.get("subscriptions".into()), subscription_hash.clone(), Value::Undefined);
            let mut method: Value = methods.get(channel.clone().clone());
            Value::Undefined;
        };
    ...
    fn handle_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut channel_parts: Value = channel.split(Value::from("."));
        let mut channel_type: Value = self.safe_value(channel_parts.clone(), Value::from(1), Value::Undefined);
        ...
        let mut method: Value = self.safe_value(v4_methods.clone(), channel_type.clone(), Value::Undefined);
        if method.clone().is_nonnullish() {
            Value::Undefined;
        };
        let mut request_id: Value = self.safe_string(message.clone(), Value::from("request_id"), Value::Undefined);
    ...
    fn get_url_by_market_type(&mut self, mut r#type: Value, mut is_inverse: Value) -> Value {
        is_inverse = is_inverse.or_default(Value::from(false));
        let mut api: Value = self.get("urls".into()).get(Value::from("api"));
        let mut url: Value = api.get(r#type.clone().clone());
    ...
    fn get_market_type_by_url(&self, mut url: Value) -> Value {
        let mut find_by: Value = Value::Json(normalize(&Value::Json(json!({
            "op-": "option",
            "delivery": "future",
            "fx": "swap"
        }))).unwrap());
        let mut keys: Value = find_by.clone().keys();
        let mut i: usize = 0;
        while i < keys.len() {
            let mut key: Value = keys.get(i.into());
            let mut value: Value = find_by.get(key.clone().clone());
    ...
    async fn subscribe_public(&mut self, mut url: Value, mut message_hash: Value, mut payload: Value, mut channel: Value, mut params: Value, mut subscription: Value) -> Value {
        ...
        if subscription.clone().is_nonnullish() {
            ...
            if !client.get("subscriptions".into()).contains_key(message_hash.clone()) {
                ...
                client.get("subscriptions".into()).set(temp_subscription_hash.clone(), message_hash.clone());
            };
        };
    ...
    async fn request_private(&mut self, mut url: Value, mut req_params: Value, mut channel: Value, mut request_id: Value) -> Value {
        ...
        let mut signature_string: Value = Value::Json(serde_json::Value::Array(vec![(event.clone()).into(), (channel.clone()).into(), (self.json(req_params.clone()).await).into(), (time.to_string()).into()])).join(Value::from("
"));
    ...
    async fn subscribe_private(&mut self, mut url: Value, mut message_hash: Value, mut payload: Value, mut channel: Value, mut params: Value, mut requires_uid: Value) -> Value {
        requires_uid = requires_uid.or_default(Value::from(false));
        self.check_required_credentials(Value::Undefined);
        // uid is required for some subscriptions only so it's not a part of required credentials
        if requires_uid.is_truthy() {
            if self.get("uid".into()).is_nullish() || self.get("uid".into()).len() == Value::from(0) {
