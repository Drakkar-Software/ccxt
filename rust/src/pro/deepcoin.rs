...
fn create_public_request(&mut self, mut market: Value, mut request_id: Value, mut topic_id: Value, mut suffix: Value, mut un_watch: Value) -> Value { Value::Undefined }

async fn watch_public(&mut self, mut market: Value, mut message_hash: Value, mut topic_id: Value, mut params: Value, mut suffix: Value) -> Value { Value::Undefined }

async fn un_watch_public(&mut self, mut market: Value, mut message_hash: Value, mut topic_id: Value, mut params: Value, mut subscription: Value, mut suffix: Value) -> Value { Value::Undefined }
...
fn parse_trade_side(&self, mut direction: Value) -> Value { Value::Undefined }

fn handle_taker_or_maker(&mut self, mut match_role: Value) -> Value { Value::Undefined }
...
fn handle_delta(&mut self, mut orderbook: Value, mut entry: Value) -> Value { Value::Undefined }
...
fn parse_position_side(&self, mut direction: Value) -> Value { Value::Undefined }

fn parse_ws_margin_mode(&self, mut margin_mode: Value) -> Value { Value::Undefined }
...
async fn fetch_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        for (api_name, method_name, path_name) in candidates {
            let rv = Deepcoin::request(self, path_name.into(), api_name.into(), method_name.into(), request.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
            if !rv.is_undefined() {
                return rv;
            }
        }
...
async fn fetch_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if let Value::Json(serde_json::Value::Object(api_map)) = Deepcoin::describe(self).get("api".into()) {
        ...
        for token in ["depth", "orderbook", "order_book"] {
            for (api_name, method_name, path_name) in &dynamic_calls {
                ...
                if p == token || p.contains(token) {
                    let rv = Deepcoin::request(self, path_name.clone().into(), api_name.clone().into(), method_name.clone().into(), request.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
                    if !rv.is_undefined() {
                        return rv;
                    }
                }
...
async fn fetch_time(&mut self, mut params: Value) -> Value {
        ...
        for (api_name, method_name, path_name) in candidates {
            let rv = Deepcoin::request(self, path_name.into(), api_name.into(), method_name.into(), params.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
            if !rv.is_undefined() {
                return rv;
            }
        }
...
async fn request(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value) -> Value {
        ...
        let urls_api = Deepcoin::describe(self).get("urls".into()).get("api".into());
        ...
        let hostname = Deepcoin::describe(self).get("hostname".into());
...
async fn fetch_status(&mut self, mut params: Value) -> Value {
        ...
        for token in ["status", "ping", "time", "system/status"] {
            for (api_name, method_name, path_name) in &dynamic_calls {
                ...
                let p = path_name.to_lowercase();
                if p == token || p.contains(token) {
                    let rv = Deepcoin::request(self, path_name.clone().into(), api_name.clone().into(), method_name.clone().into(), params.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
                    if !rv.is_undefined() {
                        return rv;
                    }
                }
