...
pub trait Phemex : Exchange {
    ...
    fn parse_swap_ticker(&mut self, mut ticker: Value, mut market: Value) -> Value {
        //
        //     {
        //         "close": 442800,
        //         "fundingRate": 10000,
    ...
    fn parse_perpetual_ticker(&mut self, mut ticker: Value, mut market: Value) -> Value {
        //
        //    [
        //        "STXUSDT",
        //        "0.64649",
    ...
    async fn watch_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut subscribe: Value = Value::Json(normalize(&Value::Json(json!({
            "method": method,
            "id": request_id,
            "params": Value::Json(serde_json::Value::Array(vec![(market.get(Value::from("id"))).into(), (self.safe_integer(self.get("timeframes".into()), timeframe.clone(), Value::Undefined).await).into()]))
        }))).unwrap());
        let mut request: Value = self.deep_extend_2(subscribe.clone(), params.clone());
    ...
    fn handle_orders(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if message.contains_key(Value::from("closed")) || message.contains_key(Value::from("fills")) || message.contains_key(Value::from("open")) {
            ...
            if message_length == Value::from(0) {
                return Value::Undefined;
            };
            let mut i: usize = 0;
            while i < message.len() {
    ...
    fn parse_ws_swap_order(&mut self, mut order: Value, mut market: Value) -> Value {
        //
        // swap
        //    {
    ...
    fn handle_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if client.get("subscriptions".into()).contains_key(id.clone()) {
            let mut method: Value = client.get("subscriptions".into()).get(id.clone().clone());
            client.get("subscriptions".into()).get(id.clone().clone());
            if method.clone() != Value::from(true) {
                Value::Undefined;
                return Value::Undefined;
            };
        };
    ...
    async fn authenticate(&mut self, mut params: Value) -> Value {
        ...
        if future.clone().is_nullish() {
            ...
            if !client.get("subscriptions".into()).contains_key(message_hash.clone()) {
                client.get("subscriptions".into()).set(subscription_hash.clone(), self.get("handleAuthenticate".into()));
            };
