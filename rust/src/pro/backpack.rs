...
pub trait Backpack : Exchange {
    ...
    async fn watch_public(&mut self, mut topics: Value, mut message_hashes: Value, mut params: Value, mut unwatch: Value) -> Value {
        ...
        unwatch = unwatch.or_default(Value::from(false));
            ...
            if message_hash.index_of(Value::from("ticker")) >= Value::from(0) {
                ...
                if message_hash.clone() == Value::from("unsubscribe:orders") {
                    ...
                    while j < keys.len() {
                        ...
                        self.get("orders".into()).get(symbol.clone().clone());
                        j += 1;
                    };
                } else {
                ...
                if message_hash.clone() == Value::from("unsubscribe:positions") {
                    ...
                    while j < keys.len() {
                        let mut symbol: Value = keys.get(j.into());
                        self.get("positions".into()).get(symbol.clone().clone());
                        j += 1;
                    };
                } else {
    ...
    async fn un_watch_tickers(&mut self, mut symbols: Value, mut params: Value) -> Value {
        ...
        return <Self as Backpack>::watch_public(self, topics.clone(), message_hashes.clone(), params.clone(), Value::from(true)).await;
    }
    ...
    fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut ohlcv: Value = self.get("ohlcvs".into()).get(symbol.clone().clone()).get(timeframe.clone().clone());
    ...
    async fn watch_trades_for_symbols(&mut self, mut symbols: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.sort_by(result.clone(), Value::from("timestamp"), Value::Undefined, Value::Undefined).await;
    }
    ...
    fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut cache: Value = self.get("trades".into()).get(symbol.clone().clone());
    ...
    async fn un_watch_orders(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        return <Self as Backpack>::watch_private(self, Value::Json(serde_json::Value::Array(vec![(topic.clone()).into()])), Value::Json(serde_json::Value::Array(vec![(message_hash.clone()).into()])), params.clone(), Value::from(true)).await;
    }
    ...
    async fn un_watch_positions(&mut self, mut symbols: Value, mut params: Value) -> Value {
        ...
        return <Self as Backpack>::watch_private(self, topics.clone(), message_hashes.clone(), params.clone(), Value::from(true)).await;
    }
