...
pub trait Probit : Exchange {
    ...
    fn parse_ws_balance(&mut self, mut message: Value) -> Value {
        //
        //     {
        //         "channel": "balance",
        //         "reset": false,
        ...
        //     }
        //
        let mut reset: Value = self.safe_bool(message.clone(), Value::from("reset"), Value::from(false));
        ...
        while i < currency_ids.len() {
            ...
            let mut entry: Value = data.get(currency_id.clone().clone());
    ...
    fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if self.safe_bool(message.clone(), Value::from("reset"), Value::from(false)).is_truthy() {
    ...
    fn handle_my_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while j < trades.len() {
            ...
            trade_symbols.set(trade.get(Value::from("symbol")), Value::from(true));
            stored.append(trade.clone());
            j += 1;
        };
        let mut unique: Value = trade_symbols.clone().keys();
        let mut unique_length: usize = unique.len();
        if unique_length == Value::from(0) {
            return Value::Undefined;
        };
        let mut i: usize = 0;
        while i < unique.len() {
    ...
    fn handle_orders(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        while i < raw_orders.len() {
            ...
            order_symbols.set(order.get(Value::from("symbol")), Value::from(true));
    ...
    async fn subscribe_public(&mut self, mut method_name: Value, mut symbol: Value, mut data_type: Value, mut filter: Value, mut params: Value) -> Value {
        ...
        if client.get("subscriptions".into()).contains_key(subscribe_hash.clone()) {
            // already subscribed
            filters = client.get("subscriptions".into()).get(subscribe_hash.clone().clone());
            if !filters.contains_key(filter.clone()) {
                // resubscribe
                client.get("subscriptions".into()).get(subscribe_hash.clone().clone());
            };
        };
        filters.set(filter.clone(), Value::from(true));
    ...
    fn handle_authenticate(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut future: Value = client.get("subscriptions".into()).get(Value::from("authenticated"));
        if result.clone() == Value::from("ok") {
            ...
            future.reject(message.clone(), Value::Undefined);
