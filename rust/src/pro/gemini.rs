...
fn handle_trades_for_multidata(&mut self, mut client: Value, mut trades: Value, mut timestamp: Value) -> Value { Value::Undefined }
...
fn handle_bids_asks_for_multidata(&mut self, mut client: Value, mut raw_bid_ask_changes: Value, mut timestamp: Value, mut nonce: Value) -> Value { Value::Undefined }

async fn helper_for_watch_multiple_construct(&mut self, mut item_hash_name: Value, mut symbols: Value, mut params: Value) -> Value { Value::Undefined }

fn handle_order_book_for_multidata(&mut self, mut client: Value, mut raw_order_book_changes: Value, mut timestamp: Value, mut nonce: Value) -> Value { Value::Undefined }

fn handle_l2_updates(&mut self, mut client: Value, mut message: Value) -> Value { Value::Undefined }
...
fn handle_subscription(&mut self, mut client: Value, mut message: Value) -> Value { Value::Undefined }
...
fn handle_error(&mut self, mut client: Value, mut message: Value) -> Value { Value::Undefined }
...
async fn fetch_trades(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        for (api_name, method_name, path_name) in candidates {
            let rv = Gemini::request(self, path_name.into(), api_name.into(), method_name.into(), request.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
            if !rv.is_undefined() {
                return rv;
            }
        }
...
async fn fetch_order_book(&mut self, mut symbol: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if let Value::Json(serde_json::Value::Object(api_map)) = Gemini::describe(self).get("api".into()) {
        ...
        for token in ["depth", "orderbook", "order_book"] {
            for (api_name, method_name, path_name) in &dynamic_calls {
                ...
                if p == token || p.contains(token) {
                    let rv = Gemini::request(self, path_name.clone().into(), api_name.clone().into(), method_name.clone().into(), request.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
                    if !rv.is_undefined() {
                        return rv;
                    }
                }
...
async fn fetch_time(&mut self, mut params: Value) -> Value {
        ...
        for (api_name, method_name, path_name) in candidates {
            let rv = Gemini::request(self, path_name.into(), api_name.into(), method_name.into(), params.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
            if !rv.is_undefined() {
                return rv;
            }
        }
...
async fn request(&mut self, mut path: Value, mut api: Value, mut method: Value, mut params: Value, mut headers: Value, mut body: Value, mut config: Value) -> Value {
        ...
        let urls_api = Gemini::describe(self).get("urls".into()).get("api".into());
        ...
        let hostname = Gemini::describe(self).get("hostname".into());
...
async fn fetch_status(&mut self, mut params: Value) -> Value {
        ...
        for token in ["status", "ping", "time", "system/status"] {
            for (api_name, method_name, path_name) in &dynamic_calls {
                ...
                let p = path_name.to_lowercase();
                if p == token || p.contains(token) {
                    let rv = Gemini::request(self, path_name.clone().into(), api_name.clone().into(), method_name.clone().into(), params.clone(), Value::Undefined, Value::Undefined, Value::Undefined).await;
                    if !rv.is_undefined() {
                        return rv;
                    }
                }
