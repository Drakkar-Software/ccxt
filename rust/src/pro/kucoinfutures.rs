...
pub trait Kucoinfutures : Exchange {
    ...
    async fn negotiate(&mut self, mut private_channel: Value, mut params: Value) -> Value {
        ...
        urls.set(connect_id.clone(), self.spawn(self.get("negotiateHelper".into()), private_channel.clone()));
        // we have to wait here otherwsie in c# will not work
        self.get("options".into()).set("urls".into(), urls.clone());
        future = urls.get(connect_id.clone().clone());
        return future.clone();
    }
    ...
    async fn negotiate_helper(&mut self, mut private_channel: Value, mut params: Value) -> Value {
        ...
        let mut connect_id: Value = if private_channel.is_truthy() { Value::from("private") } else { Value::from("public") };
            ...
            response = self.dispatch("futuresPrivatePostBulletPrivate".into(), params.clone(), Value::Undefined).await;
        } else {
            //
            //     {
            //         "code": "200000",
            ...
            //     }
            //
            response = self.dispatch("futuresPublicPostBulletPublic".into(), params.clone(), Value::Undefined).await;
        };
...
;
    ...
    async fn un_subscribe_multiple(&mut self, mut url: Value, mut message_hashes: Value, mut topic: Value, mut subscription_hashes: Value, mut params: Value, mut subscription: Value) -> Value {
        ...
        while i < subscription_hashes.len() {
            ...
            if !client.get("subscriptions".into()).contains_key(subscription_hash.clone()) {
                client.get("subscriptions".into()).set(request_id.clone(), subscription_hash.clone());
            };
            i += 1;
        };
    ...
    fn handle_ticker(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut market_id: Value = self.safe_value(data.clone(), Value::from("symbol"), Value::Undefined);
    ...
    async fn watch_multi_request(&mut self, mut method_name: Value, mut channel_name: Value, mut symbols: Value, mut params: Value) -> Value {
        ...
        let mut url: Value = <Self as Kucoinfutures>::negotiate(self, Value::from(false), Value::Undefined).await;
    ...
    fn parse_ws_bid_ask(&self, mut ticker: Value, mut market: Value) -> Value {
        ...
        let mut timestamp: Value = self.safe_integer_product(data.clone(), Value::from("ts"), Value::from(0.000001), Value::Undefined);
    ...
    async fn watch_position(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        self.load_markets(Value::Undefined, Value::Undefined).await;
        let mut url: Value = <Self as Kucoinfutures>::negotiate(self, Value::from(true), Value::Undefined).await;
        let mut market: Value = self.market(symbol.clone());
        ...
        let mut fetch_position_snapshot: Value = self.handle_option(Value::from("watchPosition"), Value::from("fetchPositionSnapshot"), Value::from(true));
        let mut await_position_snapshot: Value = self.handle_option(Value::from("watchPosition"), Value::from("awaitPositionSnapshot"), Value::from(true));
    ...
    fn get_current_position(&mut self, mut symbol: Value) -> Value {
        ...
        let mut cache: Value = self.get("positions".into()).get("hashmap".into());
        let mut symbol_cache: Value = self.safe_value(cache.clone(), symbol.clone(), Value::new_object());
        let mut values: Value = symbol_cache.clone().values();
        return self.safe_value(values.clone(), Value::from(0), Value::Undefined);
    }

    fn set_position_cache(&mut self, mut client: Value, mut symbol: Value) -> Value {
        let mut fetch_position_snapshot: Value = self.handle_option(Value::from("watchPosition"), Value::from("fetchPositionSnapshot"), Value::from(false));
        if fetch_position_snapshot.is_truthy() {
            ...
            if !client.get("futures".into()).contains_key(message_hash.clone()) {
                ...
                self.spawn(self.get("loadPositionSnapshot".into()), client.clone());
            };
        };
        Value::Undefined
    }
    ...
    fn handle_position(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut keys: Value = new_position.clone().keys();
        let mut i: usize = 0;
        while i < keys.len() {
            let mut key: Value = keys.get(i.into());
            if new_position.get(key.clone().clone()).is_nullish() {
                new_position.get(key.clone().clone());
            };
            i += 1;
        };
    ...
    fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut timeframe_id: Value = self.safe_string(parts.clone(), Value::from(1), Value::Undefined);
        ...
        let mut parsed: Value = Value::Json(serde_json::Value::Array(vec![(self.safe_integer(ohlcv.clone(), Value::from(0), Value::Undefined)).into(), (self.safe_number(ohlcv.clone(), Value::from(1), Value::Undefined)).into(), (self.safe_number(ohlcv.clone(), Value::from(2), Value::Undefined)).into(), (self.safe_number(ohlcv.clone(), Value::from(3), Value::Undefined)).into(), (self.safe_number(ohlcv.clone(), Value::from(4), Value::Undefined)).into(), (self.safe_number(ohlcv.clone(), Value::from(6), Value::Undefined)).into()]));
    ...
    fn handle_delta(&mut self, mut orderbook: Value, mut delta: Value) -> Value {
        orderbook.set("nonce".into(), self.safe_integer(delta.clone(), Value::from("sequence"), Value::Undefined));
        let mut timestamp: Value = self.safe_integer(delta.clone(), Value::from("timestamp"), Value::Undefined);
        ...
        let mut side: Value = self.safe_string(split_change.clone(), Value::from(1), Value::Undefined);
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if !self.get("orderbooks".into()).contains_key(symbol.clone()) {
            let mut subscription_args: Value = self.safe_dict(client.get("subscriptions".into()), topic.clone(), Value::new_object());
            let mut limit: Value = self.safe_integer(subscription_args.clone(), Value::from("limit"), Value::Undefined);
        ...
        let mut delta_end: Value = self.safe_integer(data.clone(), Value::from("sequence"), Value::Undefined);
        if nonce.clone().is_nullish() {
            ...
            let mut topic_symbol: Value = self.safe_string(topic_parts_new.clone(), Value::from(1), Value::Undefined);
            let mut topic_channel: Value = self.safe_string(topic_parts_new.clone(), Value::from(0), Value::Undefined);
            let mut subscriptions: Value = client.get("subscriptions".into()).keys();
            ...
            while i < subscriptions.len() {
                ...
                if key.index_of(topic_symbol.clone()) >= Value::from(0) && key.index_of(topic_channel.clone()) >= Value::from(0) {
                    subscription = client.get("subscriptions".into()).get(key.clone().clone());
                    break;
                };
                i += 1;
            };
    ...
    fn get_cache_index(&mut self, mut orderbook: Value, mut cache: Value) -> Value {
        ...
        let mut first_delta_start: Value = self.safe_integer(first_delta.clone(), Value::from("sequence"), Value::Undefined);
        ...
        while i < cache.len() {
            ...
            let mut delta_start: Value = self.safe_integer(delta.clone(), Value::from("sequence"), Value::Undefined);
    ...
    fn parse_ws_order(&self, mut order: Value, mut market: Value) -> Value {
        ...
        let mut order_type: Value = self.safe_string_lower(order.clone(), Value::from("orderType"), Value::Undefined);
        ...
        let mut timestamp: Value = self.safe_integer_product(order.clone(), Value::from("orderTime"), Value::from(0.000001), Value::Undefined);
    ...
    fn handle_order(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if symbol.clone().is_nonnullish() {
            ...
            if order.clone().is_nonnullish() {
                ...
                let mut stop_price: Value = self.safe_value(order.clone(), Value::from("stopPrice"), Value::Undefined);
    ...
    fn handle_balance(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        account.set("used".into(), self.safe_string(data.clone(), Value::from("holdBalance"), Value::Undefined));
    ...
    fn handle_balance_subscription(&mut self, mut client: Value, mut message: Value, mut subscription: Value) -> Value {
        self.spawn(self.get("fetchBalanceSnapshot".into()), client.clone());
        Value::Undefined
    }
    ...
    async fn fetch_balance_snapshot(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        //    }
        //
        let mut keys: Value = snapshot.clone().keys();
        let mut i: usize = 0;
        while i < keys.len() {
            ...
            if code.clone() != Value::from("free") && code.clone() != Value::from("used") && code.clone() != Value::from("total") && code.clone() != Value::from("timestamp") && code.clone() != Value::from("datetime") && code.clone() != Value::from("info") {
                self.get("balance".into()).set(code.clone(), snapshot.get(code.clone().clone()));
            };
            i += 1;
        };
    ...
    fn handle_error_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if data.clone() == Value::from("token is expired") {
            ...
            if client.get("url".into()).index_of(Value::from("connectId=private")) >= Value::from(0) {
                r#type = Value::from("private");
            };
            self.get("options".into()).get(Value::from("urls")).set(r#type.clone(), Value::Undefined);
        };
        self.handle_errors(Value::from(1), Value::from(""), client.get("url".into()), Value::from(""), Value::new_object(), data.clone(), message.clone(), Value::new_object(), Value::new_object());
    ...
    fn handle_subscription_status(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut is_un_sub: Value = self.safe_bool(subscription.clone(), Value::from("unsubscribe"), Value::from(false));
