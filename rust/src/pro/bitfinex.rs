...
pub trait Bitfinex : Exchange {
    ...
    async fn subscribe(&mut self, mut channel: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        if checksum.is_truthy() && channel.clone() == Value::from("book") {
            let mut sub: Value = client.get("subscriptions".into()).get(message_hash.clone().clone());
            if sub.is_truthy() && !sub.get(Value::from("checksum")).is_truthy() {
                client.get("subscriptions".into()).get(message_hash.clone().clone()).set("checksum".into(), Value::from(true));
    ...
    async fn un_subscribe(&mut self, mut channel: Value, mut topic: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut channel_id: Value = self.safe_string(client.get("subscriptions".into()), un_sub_topic.clone(), Value::Undefined);
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "event": "unsubscribe",
            "chanId": channel_id
        }))).unwrap());
        let mut un_sub_chan_msg: Value = Value::from("unsubscribe:") + channel_id.clone();
        client.get("subscriptions".into()).set(un_sub_chan_msg.clone(), sub_message_hash.clone());
    ...
    async fn un_watch_ohlcv(&mut self, mut symbol: Value, mut timeframe: Value, mut params: Value) -> Value {
        ...
        let mut channel_id: Value = self.safe_string(client.get("subscriptions".into()), sub_id.clone(), Value::Undefined);
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "event": "unsubscribe",
            "chanId": channel_id
        }))).unwrap());
    ...
    fn handle_ohlcv(&mut self, mut client: Value, mut message: Value, mut subscription: Value) -> Value {
        ...
        let mut i: usize = 0;
        while i < ohlcvs_length {
            let mut ohlcv: Value = ohlcvs.get(Value::from(ohlcvs_length) - Value::from(i) - Value::from(1).clone());
    ...
    fn handle_trades(&mut self, mut client: Value, mut message: Value, mut subscription: Value) -> Value {
        ...
        if message_length == Value::from(2) {
            // initial snapshot
            ...
            while i < length {
                ...
                let mut parsed: Value = <Self as Bitfinex>::parse_ws_trade(self, trades.get(index.clone().clone()), market.clone());
                stored.append(parsed.clone());
                i += 1;
            };
        } else {
    ...
    fn parse_ws_trade(&self, mut trade: Value, mut market: Value) -> Value {
        ...
        let mut is_public: Value = (num_fields <= Value::from(8)).into();
    ...
    fn handle_order_book(&mut self, mut client: Value, mut message: Value, mut subscription: Value) -> Value {
        ...
        if !self.get("orderbooks".into()).contains_key(symbol.clone()) {
            ...
            let mut orderbook: Value = self.get("orderbooks".into()).get(symbol.clone().clone());
            let mut deltas: Value = message.get(Value::from(1));
            let mut orderbook_item: Value = self.get("orderbooks".into()).get(symbol.clone().clone());
            if is_raw.is_truthy() {
                ...
                let mut bookside: Value = orderbook_item.get(side.clone().clone());
    ...
    fn handle_checksum(&mut self, mut client: Value, mut message: Value, mut subscription: Value) -> Value {
        ...
        while i < depth {
            ...
            if bid.clone().is_nonnullish() {
                string_array.push(self.number_to_string(bids.get(i.into()).get(id_to_check.clone().clone())));
                string_array.push(self.number_to_string(bids.get(i.into()).get(Value::from(1))));
            };
            if ask.clone().is_nonnullish() {
                string_array.push(self.number_to_string(asks.get(i.into()).get(id_to_check.clone().clone())));
    ...
    fn handle_balance(&mut self, mut client: Value, mut message: Value, mut subscription: Value) -> Value {
        ...
        while i < data.len() {
            ...
            updated_types.set(balance_type.clone(), Value::from(true));
            i += 1;
        };
    ...
    fn handle_unsubscription_status(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut sub_message_hash: Value = self.safe_string(client.get("subscriptions".into()), un_sub_channel.clone(), Value::Undefined);
        let mut subscription: Value = self.safe_dict(client.get("subscriptions".into()), Value::from("unsubscribe:") + sub_message_hash.clone(), Value::Undefined);
        client.get("subscriptions".into()).get(un_sub_channel.clone().clone());
    ...
    fn handle_subscription_status(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        //
        let mut channel_id: Value = self.safe_string(message.clone(), Value::from("chanId"), Value::Undefined);
        client.get("subscriptions".into()).set(channel_id.clone(), message.clone());
        ...
        if message.contains_key(Value::from("key")) {
            ...
            client.get("subscriptions".into()).set(sub_key_id.clone(), channel_id.clone());
        } else {
            ...
            if unified_channel.clone().is_nonnullish() {
                ...
                client.get("subscriptions".into()).set(sub_id.clone(), channel_id.clone());
            };
        };
        return message.clone();
    }
    ...
    fn handle_orders(&mut self, mut client: Value, mut message: Value, mut subscription: Value) -> Value {
        ...
        if message_type.clone() == Value::from("os") {
            ...
            if snapshot_length == Value::from(0) {
                return Value::Undefined;
            };
            let mut i: usize = 0;
            while i < data.len() {
                ...
                symbol_ids.set(symbol.clone(), Value::from(true));
    ...
    fn handle_message(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if Array::is_array(message.clone()).is_truthy() {
            ...
            let mut subscription: Value = self.safe_value(client.get("subscriptions".into()), channel_id.clone(), Value::new_object());
