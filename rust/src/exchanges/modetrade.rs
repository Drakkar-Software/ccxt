...
pub trait Modetrade : Exchange {
    ...
    async fn fetch_status(&mut self, mut params: Value) -> Value {
        ...
        if let Value::Json(serde_json::Value::Object(api_map)) = <Self as Modetrade>::describe(self).get("api".into()) {
    ...
    async fn fetch_currencies(&mut self, mut params: Value) -> Value {
        ...
        let mut response: Value = self.dispatch("v1PublicGetPublicToken".into(), params.clone(), Value::Undefined).await;
        //
        // {
        //     "success": true,
        ...
        let mut token_rows: Value = self.safe_list(data.clone(), Value::from("rows"), Value::new_array());
        let mut i: usize = 0;
        while i < token_rows.len() {
            ...
            while j < networks.len() {
                ...
                let mut network_id: Value = self.safe_string(network.clone(), Value::from("chain_id"), Value::Undefined);
                let mut precision: Value = self.parse_precision(self.safe_string(network.clone(), Value::from("decimals"), Value::Undefined));
                if precision.clone().is_nonnullish() {
                    min_precision = if min_precision.clone().is_nullish() { precision.clone() } else { Precise::string_min(precision.clone(), min_precision.clone()) };
                };
                resulting_networks.set(network_id.clone(), Value::Json(normalize(&Value::Json(json!({
                    "id": network_id.clone(),
                    ...
                    "fee": self.safe_number(network.clone(), Value::from("withdrawal_fee"), Value::Undefined),
    ...
    fn parse_trade(&self, mut trade: Value, mut market: Value) -> Value {
        ...
        let mut fee: Value = <Self as Modetrade>::parse_token_and_fee_temp(self, trade.clone(), Value::from("fee_asset"), Value::from("fee"));
    ...
    fn parse_funding_rate(&self, mut funding_rate: Value, mut market: Value) -> Value {
        ...
        return Value::Json(normalize(&Value::Json(json!({
            ...
            "interval": <Self as Modetrade>::parse_funding_interval(self, milliseconds_interval.clone())
        }))).unwrap());
    }
    ...
    async fn fetch_funding_interval(&mut self, mut symbol: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        return <Self as Modetrade>::fetch_funding_rate(self, symbol.clone(), params.clone()).await;
    }
    ...
    async fn fetch_funding_rate(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Modetrade>::parse_funding_rate(self, data.clone(), market.clone()).await;
    }
    ...
    fn parse_order(&self, mut order: Value, mut market: Value) -> Value {
        ...
        return self.safe_order(Value::Json(normalize(&Value::Json(json!({
            ...
            "status": <Self as Modetrade>::parse_order_status(self, status.clone()),
            "symbol": symbol.clone(),
            "type": <Self as Modetrade>::parse_order_type(self, order_type.clone()),
            "timeInForce": <Self as Modetrade>::parse_time_in_force(self, order_type.clone()),
    ...
    async fn create_orders(&mut self, mut orders: Value, mut params: Value) -> Value {
        ...
        while i < orders.len() {
            ...
            let mut order_request: Value = <Self as Modetrade>::create_order_request(self, market_id.clone(), r#type.clone(), side.clone(), amount.clone(), price.clone(), order_params.clone());
    ...
    async fn edit_order(&mut self, mut id: Value, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        return <Self as Modetrade>::parse_order(self, data.clone(), market.clone()).await;
    }
    ...
    async fn fetch_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        return <Self as Modetrade>::parse_order(self, orders.clone(), market.clone()).await;
    }
    ...
    fn parse_ledger_entry(&self, mut item: Value, mut currency: Value) -> Value {
        ...
        let mut fee: Value = <Self as Modetrade>::parse_token_and_fee_temp(self, item.clone(), Value::from("fee_token"), Value::from("fee_amount"));
        return self.safe_ledger_entry(Value::Json(normalize(&Value::Json(json!({
            ...
            "status": <Self as Modetrade>::parse_transaction_status(self, self.safe_string(item.clone(), Value::from("status"), Value::Undefined)),
            "amount": amount.clone(),
            ...
            "timestamp": timestamp.clone(),
            "datetime": self.iso8601(timestamp.clone()),
            "type": <Self as Modetrade>::parse_ledger_entry_type(self, self.safe_string(item.clone(), Value::from("type"), Value::Undefined)),
            "info": item.clone()
    ...
    async fn fetch_ledger(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut currency_rows: Value = <Self as Modetrade>::get_asset_history_rows(self, code.clone(), since.clone(), limit.clone(), params.clone()).await;
    ...
    fn parse_transaction(&self, mut transaction: Value, mut currency: Value) -> Value {
        ...
        let mut fee: Value = <Self as Modetrade>::parse_token_and_fee_temp(self, transaction.clone(), Value::from("fee_token"), Value::from("fee_amount"));
        ...
        return Value::Json(normalize(&Value::Json(json!({
            ...
            "currency": code.clone(),
            "status": <Self as Modetrade>::parse_transaction_status(self, self.safe_string(transaction.clone(), Value::from("status"), Value::Undefined)),
    ...
    async fn fetch_deposits_withdrawals(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut request: Value = Value::new_object();
        let mut currency_rows: Value = <Self as Modetrade>::get_asset_history_rows(self, code.clone(), since.clone(), limit.clone(), extend_2(request.clone(), params.clone())).await;
        let mut currency: Value = self.safe_value(currency_rows.clone(), Value::from(0), Value::Undefined);
        let mut rows: Value = self.safe_list(currency_rows.clone(), Value::from(1), Value::Undefined);
        //
        //     {
    ...
    async fn withdraw(&mut self, mut code: Value, mut amount: Value, mut address: Value, mut tag: Value, mut params: Value) -> Value {
        ...
        let mut withdraw_nonce: Value = <Self as Modetrade>::get_withdraw_nonce(self, params.clone()).await;
        let mut nonce: Value = <Self as Modetrade>::nonce(self);
        ...
        let mut withdraw_request: Value = Value::Json(normalize(&Value::Json(json!({
            "brokerId": self.safe_string(self.get("options".into()), Value::from("keyBrokerId"), Value::from("mode")),
        ...
        let mut signature: Value = <Self as Modetrade>::sign_message(self, msg.clone(), self.get("privateKey".into()));
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Modetrade>::parse_transaction(self, data.clone(), currency.clone()).await;
    }
    ...
    async fn fetch_leverage(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Modetrade>::parse_leverage(self, data.clone(), market.clone()).await;
    }

    async fn set_leverage(&mut self, mut leverage: Value, mut symbol: Value, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        self.load_markets(Value::Undefined, Value::Undefined).await;
        let mut is_min_leverage: Value = Value::from(leverage.clone() < Value::from(1));
        let mut is_max_leverage: Value = Value::from(leverage.clone() > Value::from(50));
        if is_min_leverage.is_truthy() || is_max_leverage.is_truthy() {
            return Value::Undefined;
        };
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
    ...
    async fn fetch_position(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        return <Self as Modetrade>::parse_position(self, data.clone(), market.clone()).await;
    }
...
impl Exchange for ModetradeImpl {
    ...
    async fn fetch_order_book(&mut self, symbol: Value, limit: Value, params: Value) -> Value { Modetrade::fetch_order_book(self, symbol, limit, params).await }
    async fn fetch_ohlcv(&mut self, symbol: Value, timeframe: Value, since: Value, limit: Value, params: Value) -> Value { Modetrade::fetch_ohlcv(self, symbol, timeframe, since, limit, params).await }
    async fn fetch_trades(&mut self, symbol: Value, since: Value, limit: Value, params: Value) -> Value { Modetrade::fetch_trades(self, symbol, since, limit, params).await }
    async fn fetch_time(&mut self, params: Value) -> Value { Modetrade::fetch_time(self, params).await }
    async fn fetch_status(&mut self, params: Value) -> Value { Modetrade::fetch_status(self, params).await }
    fn parse_trade(&self, trade: Value, market: Value) -> Value { Modetrade::parse_trade(self, trade, market) }
    fn parse_order(&self, order: Value, market: Value) -> Value { Modetrade::parse_order(self, order, market) }
    fn parse_position(&self, position: Value, market: Value) -> Value { Modetrade::parse_position(self, position, market) }
    fn parse_balance(&self, response: Value) -> Value { Modetrade::parse_balance(self, response) }
    fn parse_funding_rate(&self, contract: Value, market: Value) -> Value { Modetrade::parse_funding_rate(self, contract, market) }
    fn parse_market(&self, market: Value) -> Value { Modetrade::parse_market(self, market) }
    fn parse_transaction(&self, transaction: Value, currency: Value) -> Value { Modetrade::parse_transaction(self, transaction, currency) }
    fn parse_ledger_entry(&self, item: Value, currency: Value) -> Value { Modetrade::parse_ledger_entry(self, item, currency) }
    fn parse_leverage(&self, leverage: Value, market: Value) -> Value { Modetrade::parse_leverage(self, leverage, market) }
    fn parse_order_status(&self, status: Value) -> Value { Modetrade::parse_order_status(self, status) }
    fn parse_income(&self, income: Value, market: Value) -> Value { Modetrade::parse_income(self, income, market) }
}
...
impl ModetradeImpl {
    pub fn new(params: Value) -> Self {
        let mut rv = ModetradeImpl(match params {
            Value::Json(_) => params,
            _ => Value::new_object()
        });
        ExchangeImpl::init(&mut rv.0);

        let config_entries = Modetrade::describe(&rv);
