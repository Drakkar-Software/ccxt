...
pub trait Kucoin : Exchange {
    ...
    async fn fetch_uta_markets(&mut self, mut params: Value) -> Value {
        ...
        while i < symbols_data.len() {
            ...
            let mut is_marginable: Value = if has_margin.clone() == Value::from("1") { Value::from(true) } else { Value::from(false) };
            ...
            let mut linear: Value = Value::from(false);
            ...
            result.push(Value::Json(normalize(&Value::Json(json!({
                ...
                "type": r#type.clone(),
                "spot": spot.clone(),
                "margin": is_marginable.clone(),
                "swap": swap.clone(),
                "future": future.clone(),
                "option": Value::from(false),
                "active": Value::from(active.clone() == Value::from("1")),
    ...
    async fn load_migration_status(&mut self, mut force: Value) -> Value {
        force = force.or_default(Value::from(false));
    ...
    fn handle_hf_and_params(&mut self, mut params: Value) -> Value {
        params = params.or_default(Value::new_object());
        let mut migrated: Value = self.safe_bool(self.get("options".into()), Value::from("hf"), Value::from(false));
        let mut loaded_hf: Value = Value::Undefined;
        if migrated.clone().is_nonnullish() {
            if migrated.is_truthy() {
                loaded_hf = Value::from(true);
            } else {
                loaded_hf = Value::from(false);
            };
        };
    ...
    async fn fetch_currencies(&mut self, mut params: Value) -> Value {
        ...
        while i < currencies_data.len() {
            ...
            let mut is_fiat: Value = Value::from(chains_length == Value::from(0));
    ...
    async fn fetch_deposit_withdraw_fee(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        return <Self as Kucoin>::parse_deposit_withdraw_fee(self, data.clone(), currency.clone()).await;
    }
    ...
    fn parse_deposit_withdraw_fee(&self, mut fee: Value, mut currency: Value) -> Value {
        ...
        result.get(Value::from("networks")).set(network_code.clone(), Value::Json(normalize(&Value::Json(json!({
            "withdraw": min_withdraw_fee.clone(),
            "deposit": Value::Json(normalize(&Value::Json(json!({
                "fee": Value::Undefined,
                "percentage": Value::Undefined
            }))).unwrap())
    ...
    fn is_futures_method(&mut self, mut method_name: Value, mut params: Value) -> Value {
        ...
        return (if r#type.clone() == Value::from("contract") || r#type.clone() == Value::from("future") { (if r#type.clone() == Value::from("contract") { Value::from(r#type.clone() == Value::from("contract")) } else { Value::from(r#type.clone() == Value::from("future")) }) } else { Value::from(r#type.clone() == Value::from("futures")) });
    }
    ...
    async fn fetch_mark_prices(&mut self, mut symbols: Value, mut params: Value) -> Value {
        ...
        let mut data: Value = self.safe_list(response.clone(), Value::from("data"), Value::new_array());
        return self.parse_tickers(data.clone(), Value::Undefined, Value::Undefined).await;
    }
    ...
    async fn fetch_mark_price(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Kucoin>::parse_ticker(self, data.clone(), market.clone()).await;
    }
    ...
    async fn create_deposit_address(&mut self, mut code: Value, mut params: Value) -> Value {
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Kucoin>::parse_deposit_address(self, data.clone(), currency.clone()).await;
    }
    ...
    async fn create_orders(&mut self, mut orders: Value, mut params: Value) -> Value {
        ...
        (use_sync, params) = shift_2(self.handle_option_and_params(params.clone(), Value::from("createOrders"), Value::from("sync"), Value::from(false)));
    ...
    fn market_order_amount_to_precision(&self, mut symbol: Value, mut amount: Value) -> Value {
        let mut market: Value = self.market(symbol.clone());
    ...
    fn create_order_request(&mut self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut params: Value) -> Value {
        ...
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "clientOid": client_order_id.clone(),
            "side": side.clone(),
            "symbol": market.get(Value::from("id")),
            "type": r#type.clone()
        }))).unwrap());
        ...
        if is_trigger_order.is_truthy() {
            ...
            if margin_mode.clone() == Value::from("isolated") {
                ...
                request.set("tradeType".into(), self.get("options".into()).get(Value::from("marginModes")).get(margin_mode.clone().clone()));
            };
        ...
        (post_only, params) = shift_2(self.handle_post_only(Value::from(r#type.clone() == Value::from("market")), Value::from(false), params.clone()));
    ...
    async fn cancel_all_orders(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        let mut trigger: Value = self.safe_bool_2(params.clone(), Value::from("trigger"), Value::from("stop"), Value::from(false));
        let mut hf: Value = Value::Undefined;
        (hf, params) = shift_2(<Self as Kucoin>::handle_hf_and_params(self, params.clone()));
    ...
    async fn fetch_orders_by_status(&mut self, mut status: Value, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if list_data.clone().is_nonnullish() {
            return self.parse_orders(list_data.clone(), market.clone(), since.clone(), limit.clone(), Value::Undefined).await;
        };
    ...
    async fn fetch_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        return <Self as Kucoin>::parse_order(self, response_data.clone(), market.clone()).await;
    }
    ...
    fn parse_order(&self, mut order: Value, mut market: Value) -> Value {
        ...
        let mut cancel_exist: Value = self.safe_bool(order.clone(), Value::from("cancelExist"), Value::from(false));
        let mut response_stop: Value = self.safe_string(order.clone(), Value::from("stop"), Value::Undefined);
        let mut trigger: Value = Value::from(response_stop.clone().is_nonnullish());
        let mut stop_triggered: Value = self.safe_bool(order.clone(), Value::from("stopTriggered"), Value::from(false));
        ...
        if is_active.clone().is_nonnullish() {
            if is_active.clone() == Value::from(true) {
    ...
    async fn withdraw(&mut self, mut code: Value, mut amount: Value, mut address: Value, mut tag: Value, mut params: Value) -> Value {
        ...
        let mut request: Value = Value::Json(normalize(&Value::Json(json!({
            "currency": currency.get(Value::from("id")),
            "toAddress": address.clone(),
            "withdrawType": Value::from("ADDRESS")
        }))).unwrap());
        // 'memo': tag,
        ...
        (include_fee, params) = shift_2(self.handle_option_and_params(params.clone(), Value::from("withdraw"), Value::from("includeFee"), Value::from(false)));
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Kucoin>::parse_transaction(self, data.clone(), currency.clone()).await;
    }
    ...
    fn parse_transaction_status(&self, mut status: Value) -> Value {
        let mut statuses: Value = Value::Json(normalize(&Value::Json(json!({
            ...
            "WALLET_PROCESSING": Value::from("pending"),
    ...
    fn parse_transaction(&self, mut transaction: Value, mut currency: Value) -> Value {
        ...
        if txid.clone().is_nonnullish() {
            ...
            if num_txid_parts > Value::from(1) {
                if address.clone().is_nullish() {
                    if txid_parts.get(Value::from(1)).len() > Value::from(1) {
    ...
    fn is_hf_or_mining(&mut self, mut from_id: Value, mut to_id: Value) -> Value {
        return (if from_id.clone() == Value::from("trade_hf") || to_id.clone() == Value::from("trade_hf") || from_id.clone() == Value::from("pool") { (if from_id.clone() == Value::from("trade_hf") || to_id.clone() == Value::from("trade_hf") { (if from_id.clone() == Value::from("trade_hf") { Value::from(from_id.clone() == Value::from("trade_hf")) } else { Value::from(to_id.clone() == Value::from("trade_hf")) }) } else { Value::from(from_id.clone() == Value::from("pool")) }) } else { Value::from(to_id.clone() == Value::from("pool")) });
    }
    ...
    fn parse_transfer(&self, mut transfer: Value, mut currency: Value) -> Value {
        ...
        let mut is_ledger_entry: Value = Value::from(biz_type.clone().is_nonnullish());
        ...
        return Value::Json(normalize(&Value::Json(json!({
            ...
            "fromAccount": account_from.clone(),
            "toAccount": account_to.clone(),
    ...
    fn parse_ledger_entry_type(&self, mut r#type: Value) -> Value {
        let mut types: Value = Value::Json(normalize(&Value::Json(json!({
            "Assets Transferred in After Upgrading": Value::from("transfer"),
            "Deposit": Value::from("transaction"),
            "Withdrawal": Value::from("transaction"),
            "Transfer": Value::from("transfer"),
            "Trade_Exchange": Value::from("trade"),
            "KuCoin Bonus": Value::from("bonus"),
            "Referral Bonus": Value::from("referral"),
            "Rewards": Value::from("bonus"),
            "Airdrop/Fork": Value::from("airdrop"),
            "Other rewards": Value::from("bonus"),
            "Fee Rebate": Value::from("rebate"),
            "Buy Crypto": Value::from("trade"),
            "Sell Crypto": Value::from("sell"),
            "Public Offering Purchase": Value::from("trade"),
            "Refunded Fees": Value::from("fee"),
            "KCS Pay Fees": Value::from("fee"),
            "Margin Trade": Value::from("trade"),
            "Loans": Value::from("Loans"),
            "Instant Exchange": Value::from("trade"),
            "Sub-account transfer": Value::from("transfer"),
            "Liquidation Fees": Value::from("fee")
        }))).unwrap());
    ...
    fn parse_ledger_entry(&self, mut item: Value, mut currency: Value) -> Value {
        ...
        if context.clone().is_nonnullish() && context.clone() != Value::from("") {
            ...
            if !__try_error_0.is_undefined() {
                let mut exc = __try_error_0.clone();
                reference_id = context.clone();
            };
        };
        let mut fee: Value = Value::Undefined;
    ...
    async fn fetch_ledger(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        if data_list.clone().is_nonnullish() {
            return self.parse_ledger(data_list.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined).await;
        };
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::Undefined);
        let mut items: Value = self.safe_list(data.clone(), Value::from("items"), Value::new_array());
        return self.parse_ledger(items.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined).await;
    }
    ...
    async fn fetch_borrow_interest(&mut self, mut code: Value, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.filter_by_symbol_since_limit(filtered_by_currency.clone(), symbol.clone(), since.clone(), limit.clone(), Value::Undefined).await;
    }
    ...
    fn parse_borrow_interest(&self, mut info: Value, mut market: Value) -> Value {
        ...
        return Value::Json(normalize(&Value::Json(json!({
            ...
            "amountBorrowed": amount_borrowed.clone(),
    ...
    async fn fetch_borrow_rate_histories(&mut self, mut codes: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return <Self as Kucoin>::parse_borrow_rate_histories(self, rows.clone(), codes.clone(), since.clone(), limit.clone()).await;
    }
    ...
    fn parse_borrow_rate_histories(&self, mut response: Value, mut codes: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        while i < response.len() {
            ...
            if codes.clone().is_nullish() || self.in_array(code.clone(), codes.clone()).is_truthy() {
                ...
                let mut borrow_rate_histories_code: Value = borrow_rate_histories.get(code.clone().clone());
    ...
    async fn borrow_cross_margin(&mut self, mut code: Value, mut amount: Value, mut params: Value) -> Value {
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Kucoin>::parse_margin_loan(self, data.clone(), currency.clone()).await;
    }
    ...
    async fn set_leverage(&mut self, mut leverage: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        request.set("isIsolated".into(), Value::from(margin_mode.clone() == Value::from("isolated")));
    ...
    async fn fetch_funding_rate(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        //
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        return <Self as Kucoin>::parse_funding_rate(self, data.clone(), market.clone()).await;
    }
    ...
    async fn fetch_funding_rate_history(&mut self, mut symbol: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        return self.parse_funding_rate_histories(result.clone(), market.clone(), since.clone(), limit.clone()).await;
    }
    ...
    async fn fetch_transfers(&mut self, mut code: Value, mut since: Value, mut limit: Value, mut params: Value) -> Value {
        ...
        let mut data: Value = self.safe_dict(response.clone(), Value::from("data"), Value::new_object());
        let mut items: Value = self.safe_list(data.clone(), Value::from("items"), Value::new_array());
        return self.parse_transfers(items.clone(), currency.clone(), since.clone(), limit.clone(), Value::Undefined).await;
    }
...
impl Exchange for KucoinImpl {
    fn describe(&self) -> Value { Kucoin::describe(self) }
    async fn fetch_ticker(&mut self, symbol: Value, params: Value) -> Value { Kucoin::fetch_ticker(self, symbol, params).await }
    async fn fetch_tickers(&mut self, symbols: Value, params: Value) -> Value { Kucoin::fetch_tickers(self, symbols, params).await }
    async fn fetch_order_book(&mut self, symbol: Value, limit: Value, params: Value) -> Value { Kucoin::fetch_order_book(self, symbol, limit, params).await }
    async fn fetch_ohlcv(&mut self, symbol: Value, timeframe: Value, since: Value, limit: Value, params: Value) -> Value { Kucoin::fetch_ohlcv(self, symbol, timeframe, since, limit, params).await }
    async fn fetch_trades(&mut self, symbol: Value, since: Value, limit: Value, params: Value) -> Value { Kucoin::fetch_trades(self, symbol, since, limit, params).await }
    async fn fetch_time(&mut self, params: Value) -> Value { Kucoin::fetch_time(self, params).await }
    async fn fetch_status(&mut self, params: Value) -> Value { Kucoin::fetch_status(self, params).await }
    fn parse_trade(&self, trade: Value, market: Value) -> Value { Kucoin::parse_trade(self, trade, market) }
    fn parse_order(&self, order: Value, market: Value) -> Value { Kucoin::parse_order(self, order, market) }
    fn parse_ticker(&self, ticker: Value, market: Value) -> Value { Kucoin::parse_ticker(self, ticker, market) }
    fn parse_funding_rate(&self, contract: Value, market: Value) -> Value { Kucoin::parse_funding_rate(self, contract, market) }
    fn parse_transaction(&self, transaction: Value, currency: Value) -> Value { Kucoin::parse_transaction(self, transaction, currency) }
    fn parse_transfer(&self, transfer: Value, currency: Value) -> Value { Kucoin::parse_transfer(self, transfer, currency) }
    fn parse_deposit_address(&self, deposit_address: Value, currency: Value) -> Value { Kucoin::parse_deposit_address(self, deposit_address, currency) }
    fn parse_ledger_entry(&self, item: Value, currency: Value) -> Value { Kucoin::parse_ledger_entry(self, item, currency) }
}
