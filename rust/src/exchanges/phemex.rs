...
impl Exchange for PhemexImpl {
    ...
    async fn fetch_ticker(&mut self, symbol: Value, params: Value) -> Value { Phemex::fetch_ticker(self, symbol, params).await }
    async fn fetch_tickers(&mut self, symbols: Value, params: Value) -> Value { Phemex::fetch_tickers(self, symbols, params).await }
    async fn fetch_order_book(&mut self, symbol: Value, limit: Value, params: Value) -> Value { Phemex::fetch_order_book(self, symbol, limit, params).await }
    async fn fetch_ohlcv(&mut self, symbol: Value, timeframe: Value, since: Value, limit: Value, params: Value) -> Value { Phemex::fetch_ohlcv(self, symbol, timeframe, since, limit, params).await }
    async fn fetch_trades(&mut self, symbol: Value, since: Value, limit: Value, params: Value) -> Value { Phemex::fetch_trades(self, symbol, since, limit, params).await }
    fn parse_trade(&self, trade: Value, market: Value) -> Value { Phemex::parse_trade(self, trade, market) }
    fn parse_order(&self, order: Value, market: Value) -> Value { Phemex::parse_order(self, order, market) }
    fn parse_ticker(&self, ticker: Value, market: Value) -> Value { Phemex::parse_ticker(self, ticker, market) }
    fn parse_position(&self, position: Value, market: Value) -> Value { Phemex::parse_position(self, position, market) }
    fn parse_funding_rate(&self, contract: Value, market: Value) -> Value { Phemex::parse_funding_rate(self, contract, market) }
    fn parse_transaction(&self, transaction: Value, currency: Value) -> Value { Phemex::parse_transaction(self, transaction, currency) }
    fn parse_transfer(&self, transfer: Value, currency: Value) -> Value { Phemex::parse_transfer(self, transfer, currency) }
    fn parse_open_interest(&self, interest: Value, market: Value) -> Value { Phemex::parse_open_interest(self, interest, market) }
    fn parse_order_status(&self, status: Value) -> Value { Phemex::parse_order_status(self, status) }
    fn parse_conversion(&self, conversion: Value, from_currency: Value, to_currency: Value) -> Value { Phemex::parse_conversion(self, conversion, from_currency, to_currency) }
}
