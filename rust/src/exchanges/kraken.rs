...
impl Exchange for KrakenImpl {
    fn describe(&self) -> Value { Kraken::describe(self) }
    async fn fetch_ticker(&mut self, symbol: Value, params: Value) -> Value { Kraken::fetch_ticker(self, symbol, params).await }
    async fn fetch_tickers(&mut self, symbols: Value, params: Value) -> Value { Kraken::fetch_tickers(self, symbols, params).await }
    async fn fetch_order_book(&mut self, symbol: Value, limit: Value, params: Value) -> Value { Kraken::fetch_order_book(self, symbol, limit, params).await }
    async fn fetch_ohlcv(&mut self, symbol: Value, timeframe: Value, since: Value, limit: Value, params: Value) -> Value { Kraken::fetch_ohlcv(self, symbol, timeframe, since, limit, params).await }
    async fn fetch_trades(&mut self, symbol: Value, since: Value, limit: Value, params: Value) -> Value { Kraken::fetch_trades(self, symbol, since, limit, params).await }
    async fn fetch_time(&mut self, params: Value) -> Value { Kraken::fetch_time(self, params).await }
    async fn fetch_status(&mut self, params: Value) -> Value { Kraken::fetch_status(self, params).await }
    fn parse_trade(&self, trade: Value, market: Value) -> Value { Kraken::parse_trade(self, trade, market) }
    fn parse_order(&self, order: Value, market: Value) -> Value { Kraken::parse_order(self, order, market) }
    fn parse_ticker(&self, ticker: Value, market: Value) -> Value { Kraken::parse_ticker(self, ticker, market) }
    fn parse_position(&self, position: Value, market: Value) -> Value { Kraken::parse_position(self, position, market) }
    fn parse_balance(&self, response: Value) -> Value { Kraken::parse_balance(self, response) }
    fn parse_transaction(&self, transaction: Value, currency: Value) -> Value { Kraken::parse_transaction(self, transaction, currency) }
    fn parse_transfer(&self, transfer: Value, currency: Value) -> Value { Kraken::parse_transfer(self, transfer, currency) }
    fn parse_deposit_address(&self, deposit_address: Value, currency: Value) -> Value { Kraken::parse_deposit_address(self, deposit_address, currency) }
    fn parse_ledger_entry(&self, item: Value, currency: Value) -> Value { Kraken::parse_ledger_entry(self, item, currency) }
    fn parse_order_status(&self, status: Value) -> Value { Kraken::parse_order_status(self, status) }
}
