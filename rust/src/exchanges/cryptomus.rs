...
pub trait Cryptomus : Exchange {
    ...
    fn parse_order(&self, mut order: Value, mut market: Value) -> Value {
        ...
        return self.safe_order(Value::Json(normalize(&Value::Json(json!({
            ...
            "amount": amount.clone(),
            "cost": cost.clone(),
            "average": average_filled_price.clone(),
    ...
    fn parse_order_status(&self, mut status: Value) -> Value {
        let mut statuses: Value = Value::Json(normalize(&Value::Json(json!({
            ...
            "partially_completed": Value::from("open"),
...
impl Exchange for CryptomusImpl {
    ...
    async fn fetch_tickers(&mut self, symbols: Value, params: Value) -> Value { Cryptomus::fetch_tickers(self, symbols, params).await }
    async fn fetch_order_book(&mut self, symbol: Value, limit: Value, params: Value) -> Value { Cryptomus::fetch_order_book(self, symbol, limit, params).await }
    async fn fetch_trades(&mut self, symbol: Value, since: Value, limit: Value, params: Value) -> Value { Cryptomus::fetch_trades(self, symbol, since, limit, params).await }
    fn parse_trade(&self, trade: Value, market: Value) -> Value { Cryptomus::parse_trade(self, trade, market) }
    fn parse_order(&self, order: Value, market: Value) -> Value { Cryptomus::parse_order(self, order, market) }
    fn parse_ticker(&self, ticker: Value, market: Value) -> Value { Cryptomus::parse_ticker(self, ticker, market) }
    fn parse_balance(&self, response: Value) -> Value { Cryptomus::parse_balance(self, response) }
    fn parse_market(&self, market: Value) -> Value { Cryptomus::parse_market(self, market) }
    fn parse_order_status(&self, status: Value) -> Value { Cryptomus::parse_order_status(self, status) }
}
