...
pub trait Btcmarkets : Exchange {
    ...
    fn parse_transaction_status(&self, mut status: Value) -> Value {
        let mut statuses: Value = Value::Json(normalize(&Value::Json(json!({
            ...
            "Pending Authorization": Value::from("pending"),
    ...
    async fn fetch_ticker_2(&mut self, mut symbol: Value, mut params: Value) -> Value {
        ...
        return <Self as Btcmarkets>::parse_ticker(self, response.clone(), market.clone()).await;
    }
    ...
    /// * `params` {object} - 
    fn calculate_fee(&self, mut symbol: Value, mut r#type: Value, mut side: Value, mut amount: Value, mut price: Value, mut taker_or_maker: Value, mut params: Value) -> Value {
        ...
        let mut rate: Value = market.get(taker_or_maker.clone().clone());
    ...
    fn parse_order_status(&self, mut status: Value) -> Value {
        let mut statuses: Value = Value::Json(normalize(&Value::Json(json!({
            ...
            "Partially Matched": Value::from("open"),
            "Fully Matched": Value::from("closed"),
            "Cancelled": Value::from("canceled"),
            "Partially Cancelled": Value::from("canceled"),
            "Failed": Value::from("rejected")
        }))).unwrap());
        return self.safe_string(statuses.clone(), status.clone(), status.clone());
    }
    ...
    async fn fetch_order(&mut self, mut id: Value, mut symbol: Value, mut params: Value) -> Value {
        ...
        return <Self as Btcmarkets>::parse_order(self, response.clone(), Value::Undefined).await;
    }
    ...
    async fn withdraw(&mut self, mut code: Value, mut amount: Value, mut address: Value, mut tag: Value, mut params: Value) -> Value {
        ...
        //      }
        //
        return <Self as Btcmarkets>::parse_transaction(self, response.clone(), currency.clone()).await;
    }
...
impl Exchange for BtcmarketsImpl {
    ...
    async fn fetch_ticker(&mut self, symbol: Value, params: Value) -> Value { Btcmarkets::fetch_ticker(self, symbol, params).await }
    async fn fetch_order_book(&mut self, symbol: Value, limit: Value, params: Value) -> Value { Btcmarkets::fetch_order_book(self, symbol, limit, params).await }
    async fn fetch_ohlcv(&mut self, symbol: Value, timeframe: Value, since: Value, limit: Value, params: Value) -> Value { Btcmarkets::fetch_ohlcv(self, symbol, timeframe, since, limit, params).await }
    async fn fetch_trades(&mut self, symbol: Value, since: Value, limit: Value, params: Value) -> Value { Btcmarkets::fetch_trades(self, symbol, since, limit, params).await }
    async fn fetch_time(&mut self, params: Value) -> Value { Btcmarkets::fetch_time(self, params).await }
    fn parse_trade(&self, trade: Value, market: Value) -> Value { Btcmarkets::parse_trade(self, trade, market) }
    fn parse_order(&self, order: Value, market: Value) -> Value { Btcmarkets::parse_order(self, order, market) }
    fn parse_ticker(&self, ticker: Value, market: Value) -> Value { Btcmarkets::parse_ticker(self, ticker, market) }
    fn parse_balance(&self, response: Value) -> Value { Btcmarkets::parse_balance(self, response) }
    fn parse_market(&self, market: Value) -> Value { Btcmarkets::parse_market(self, market) }
    fn parse_transaction(&self, transaction: Value, currency: Value) -> Value { Btcmarkets::parse_transaction(self, transaction, currency) }
    fn parse_order_status(&self, status: Value) -> Value { Btcmarkets::parse_order_status(self, status) }
}
