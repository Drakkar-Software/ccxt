...
from ccxt.abstract.ob_okx import ImplicitAPI
from ccxt.base.types import Any, Bool, Market, Order, Str, FundingRate
from typing import List
from ccxt.base.errors import PermissionDenied
from ccxt.base.errors import OBIPWhitelistError


class ob_okx(okx, ImplicitAPI):

    def describe(self) -> Any:
        return self.deep_extend(super(ob_okx, self).describe(), {
            'id': 'ob_okx',
            'name': 'OKX',
            'certified': False,
            'urls': {
    ...
    def parse_order(self, order: dict, market: Market = None) -> Order:
        ...
        parsed = super(ob_okx, self).parse_order(order, market)
        ...
        if type != 'market' and type != 'limit':
            ...
            if slTrigger is not None and tpTrigger is not None:
                ...
                self.log('ob_okx.parseOrder', 'Unsupported OKX OCO(stop loss & take profit in a single order): ' + self.json(parsed))
                updated = 'unsupported'
                ...
                self.log('ob_okx.parseOrder', 'Unknown ob_okx order type, order: ' + self.json(parsed))
    ...
    def parse_funding_rate(self, data, market: Market = None) -> FundingRate:
        ...
        parsed = super(ob_okx, self).parse_funding_rate(data, market)
        nextFundingTimestamp = self.safe_integer(parsed, 'nextFundingTimestamp')
        if nextFundingTimestamp is not None:
            intervalMs = 8 * 60 * 60 * 1000
            previousFundingTimestamp = max(nextFundingTimestamp - intervalMs, 0)
