...
class ob_kucoinfutures(kucoinfutures, ImplicitAPI):
    ...
    def fetch_open_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if effectiveLimit is None:
            ...
            effectiveLimit = 200
        return super(ob_kucoinfutures, self).fetch_open_orders(symbol, since, effectiveLimit, params)
    ...
    def parse_order(self, order: dict, market: Market = None) -> Order:
        ...
        parsed = super(ob_kucoinfutures, self).parse_order(order, market)
    ...
    def adapt_kucoin_order_type(self, parsed: dict) -> dict:
        ...
        if triggerDirection == 'loss':
            ...
            self.log('ob_kucoinfutures.parseOrder', 'Unhandled ob_kucoin trigger direction: ' + triggerDirection + ', order: ' + self.json(parsed))
            return parsed
        if isStopLoss:
            triggerAbove = (side == 'buy')
        if isStopEntry:
            self.log('ob_kucoinfutures.parseOrder', 'Unhandled ob_kucoin stop_entry order: ' + self.json(parsed))
