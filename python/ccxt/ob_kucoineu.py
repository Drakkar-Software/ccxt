...
from ccxt.kucoineu import kucoineu
from ccxt.abstract.ob_kucoineu import ImplicitAPI
...
class ob_kucoineu(kucoineu, ImplicitAPI):

    def describe(self) -> Any:
        return self.deep_extend(super(ob_kucoineu, self).describe(), {
            'id': 'ob_kucoineu',
            'name': 'KuCoin EU',
            'certified': False,
            'urls': {
 ...
 @param market Parsed spot market structure.
    ...
    def fetch_markets(self, params={}) -> List[Market]:
        markets = super(ob_kucoineu, self).fetch_markets(params)
        result = []
    ...
    def fetch_permissions(self, params={}) -> List[str]:
        rights = self.ob_fetch_permissions_imaginary_cancel('12345', 'BTC/USDT', params, 'pair')
        filteredRights = []
        for idx in range(0, len(rights)):
            if rights[idx] != 'futuresTrading':
                filteredRights.append(rights[idx])
    ...
    def fetch_open_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if effectiveLimit is None:
            ...
            effectiveLimit = 200
        return super(ob_kucoineu, self).fetch_open_orders(symbol, since, effectiveLimit, params)
    ...
    def parse_order(self, order: dict, market: Market = None) -> Order:
        ...
        parsed = super(ob_kucoineu, self).parse_order(order, market)
    ...
    def parse_trade(self, trade: dict, market: Market = None) -> Trade:
        ...
        parsed = super(ob_kucoineu, self).parse_trade(trade, market)
        self.adapt_kucoin_order_type(parsed)
        self.ensure_kucoin_fee(parsed)
        return parsed
    ...
    def parse_funding_rate(self, data, market: Market = None) -> FundingRate:
        ...
        parsed = super(ob_kucoineu, self).parse_funding_rate(data, market)
    ...
    def adapt_kucoin_order_type(self, parsed: dict) -> dict:
        ...
        if triggerDirection == 'loss':
            ...
            self.log('ob_kucoineu.parseOrder', 'Unhandled ob_kucoineu trigger direction: ' + triggerDirection + ', order: ' + self.json(parsed))
            return parsed
        if isStopLoss:
            triggerAbove = (side == 'buy')
        if isStopEntry:
            self.log('ob_kucoineu.parseOrder', 'Unhandled ob_kucoineu stop_entry order: ' + self.json(parsed))
