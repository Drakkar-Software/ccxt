...
class alchemy(Exchange, ImplicitAPI):
    ...
    async def eth_call(self, rpcUrl: str, toAddress: str, calldata: str, params={}) -> str:
        requestBody = {
            'jsonrpc': '2.0',
            'id': 1,
            'method': 'eth_call',
    ...
    async def fetch_token_decimals(self, networkCode: str, tokenAddress: str, params={}) -> int:
        ...
        resultHex = await self.eth_call(rpcUrl, tokenAddress, self.get_decimals_selector(), params)
    ...
    async def build_quoter_calldata_for_market(self, market: Market, params={}) -> dict:
        ...
        baseId = self.safe_string(market, 'baseId')
        quoteId = self.safe_string(market, 'quoteId')
        baseDecimals = await self.fetch_token_decimals(networkCode, baseId, params)
    ...
    async def fetch_quoter_amount_out(self, market: Market, params={}) -> dict:
        quoteRequest = await self.build_quoter_calldata_for_market(market, params)
        ...
        rpcUrl = self.get_rpc_url(networkCode)
        resultHex = await self.eth_call(rpcUrl, quoter, calldata, params)
        amountOut = self.decode_quoter_amount_out(resultHex)
        quoteDecimals = await self.fetch_token_decimals(networkCode, quoteId, params)
    ...
    async def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        quoteInfo = await self.fetch_quoter_amount_out(market, params)
