...
from ccxt.async_support.trocador import trocador
from ccxt.abstract.ob_trocador import ImplicitAPI
from ccxt.base.types import Any, Market, Order, Ticker
from typing import List


class ob_trocador(trocador, ImplicitAPI):

    def describe(self) -> Any:
        return self.deep_extend(super(ob_trocador, self).describe(), {
            'id': 'ob_trocador',
            'name': 'Trocador',
            'certified': False,
            'urls': {
    ...
    async def ob_load_markets_for_symbols(self, symbols: List[str], reload=False, params={}) -> List[dict]:
        ...
        """
        await super(ob_trocador, self).ob_load_markets_for_symbols(symbols, reload, params)
    ...
    async def ob_get_exchange_trading_providers(self, params={}) -> Any:
        """
        ...
        """
        return await self.publicGetExchanges(params)

    def parse_order(self, order: dict, market: Market = None) -> Order:
        parsed = super(ob_trocador, self).parse_order(order, market)
        self.adapt_trocador_address_provider_to_esov_address_from(parsed)
        self.ensure_trocador_fee(parsed)
        return parsed

    def parse_ticker(self, ticker: dict, market: Market = None) -> Ticker:
        parsed = super(ob_trocador, self).parse_ticker(ticker, market)
        self.ensure_trocador_ticker_timestamp(parsed)
        return parsed

    def adapt_trocador_address_provider_to_esov_address_from(self, parsed: dict) -> dict:
        info = self.safe_dict(parsed, 'info', {})
        addressFrom = self.safe_string(info, 'address_provider')
        if addressFrom is not None:
            existing = self.safe_dict(parsed, 'esov', {})
            parsed['esov'] = self.extend(existing, {'address_from': addressFrom})
        return parsed

    def ensure_trocador_fee(self, parsed: dict) -> dict:
    ...
    def ensure_trocador_ticker_timestamp(self, parsed: dict) -> dict:
