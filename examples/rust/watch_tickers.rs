...
// Source: examples/ts/watch-tickers.ts
...
async fn main() {
    ...
    let rv = Binance::watch_tickers(&mut exchange, symbol.clone(), Value::Undefined).await;
    println!("watchTickers: {}", normalize(&rv).map(|v| v.to_string()).unwrap_or_else(|| "undefined".into()));
}
