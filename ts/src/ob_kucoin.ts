...
export default class ob_kucoin extends kucoin {
    describe (): any {
    ...
    adaptKucoinOrderType (parsed: Dict): Dict {
        ...
        if (triggerDirection === 'loss') {
            ...
            this.log ('ob_kucoin.parseOrder', 'Unhandled ob_kucoin trigger direction: ' + triggerDirection + ', order: ' + this.json (parsed));
            return parsed;
        }
        if (isStopLoss) {
        ...
        if (isStopEntry) {
            this.log ('ob_kucoin.parseOrder', 'Unhandled ob_kucoin stop_entry order: ' + this.json (parsed));
        }
