...
import exolix from './exolix.js';
import type { Dict, Market, Order, Ticker } from './base/types.js';

//  ---------------------------------------------------------------------------

export default class ob_exolix extends exolix {
    describe (): any {
    ...
    /**
     * @method
     * @name ob_exolix#obLoadMarketsForSymbols
    ...
    parseOrder (order: Dict, market: Market = undefined): Order {
        ...
        this.adaptExolixDepositToEsovAddressFrom (parsed);
        this.ensureExolixFee (parsed);
        return parsed as Order;
    }

    parseTicker (ticker: Dict, market: Market = undefined): Ticker {
        const parsed = super.parseTicker (ticker, market) as Dict;
        this.ensureExolixTickerTimestamp (parsed);
        return parsed as Ticker;
    }

    adaptExolixDepositToEsovAddressFrom (parsed: Dict): Dict {
        const info = this.safeDict (parsed, 'info', {});
        const addressFrom = this.safeString (info, 'depositAddress');
    ...
    ensureExolixFee (parsed: Dict): Dict {
    ...
    ensureExolixTickerTimestamp (parsed: Dict): Dict {
