...
import kucoineu from './kucoineu.js';
 ...
 * @augments kucoineu
 */
export default class ob_kucoineu extends kucoineu {
    describe (): any {
    ...
    obApplyKucoinMinFundsCostMin (market: Market): Market {
        ...
        const info = this.safeDict (marketDict, 'info', {});
        const minFundsStr = this.safeString (info, 'minFunds');
        if (minFundsStr === undefined || minFundsStr === '') {
            return market;
        }
        const minFunds = this.parseNumber (minFundsStr);
        if (minFunds === undefined) {
            return market;
        }
        const limits = this.safeDict (marketDict, 'limits', {});
        const cost = this.safeDict (limits, 'cost', {});
        const currentMin = this.safeNumber (cost, 'min');
        if (currentMin !== undefined) {
            cost['min'] = Math.max (currentMin, minFunds);
        } else {
            cost['min'] = minFunds;
        }
        return market;
    ...
    async fetchMarkets (params = {}): Promise<Market[]> {
        ...
        for (let i = 0; i < markets.length; i++) {
            result.push (this.obApplyKucoinMinFundsCostMin (markets[i]));
        }
        return result;
    }

    async fetchPermissions (params = {}): Promise<string[]> {
        const rights = await this.obFetchPermissionsImaginaryCancel ('12345', 'BTC/USDT', params, 'pair');
        const filteredRights = [];
        for (let idx = 0; idx < rights.length; idx++) {
            if (rights[idx] !== 'futuresTrading') {
                filteredRights.push (rights[idx]);
            }
        }
    ...
    adaptKucoinOrderType (parsed: Dict): Dict {
        ...
        if (triggerDirection === 'loss') {
            ...
            this.log ('ob_kucoineu.parseOrder', 'Unhandled ob_kucoineu trigger direction: ' + triggerDirection + ', order: ' + this.json (parsed));
            return parsed;
        }
        if (isStopLoss) {
        ...
        if (isStopEntry) {
            this.log ('ob_kucoineu.parseOrder', 'Unhandled ob_kucoineu stop_entry order: ' + this.json (parsed));
        }
